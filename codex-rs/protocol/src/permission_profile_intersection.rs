...
pub fn intersect_effective_permission_profiles(
    ...
    if let Some(temp) = common_temp {
        ...
        if let Ok(temp_dir) =
            AbsolutePathBuf::try_from(temp_dir.canonicalize().map_err(|error| {
                PermissionIntersectionError::UnsupportedPath(format!("tempdir: {error}"))
            })?)
        {
            let restriction = FileSystemSandboxEntry::new(
                FileSystemPath::Path {
                    path: temp_dir.into(),
                },
                FileSystemAccessMode::Read,
            );
            if !intersection.entries.contains(&restriction) {
                intersection.entries.insert(0, restriction);
            }
        }
