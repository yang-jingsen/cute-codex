...
use crate::items::CutexParticipantPresentation;
use crate::items::CutexUiActivity;
use crate::items::CutexUiActivityDelivery;
use crate::items::TaskServiceMessagePresentation;
...
pub enum Op {
    ...
    /// Emit a strictly UI-only Cutex activity without touching model context. Activity on an
    /// authoritative native turn is durable; activity on the reserved Cutex UI-only lane is live
    /// presentation transport and is not persisted.
    CutexUiActivity {
        turn_id: String,
        delivery: CutexUiActivityDelivery,
        activity: CutexUiActivity,
        reply: oneshot::Sender<CodexResult<CutexUiActivityDisposition>>,
    },
...
#[derive(Debug, Copy, Clone, Serialize, Deserialize, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum InterAgentDeliveryMode {
    AfterTurn,
    Soon,
    Passive,
    Interrupt,
...
impl InterAgentDeliveryMode {
    pub fn from_legacy_trigger_turn(trigger_turn: bool) -> Self {
        if trigger_turn {
            Self::Soon
        } else {
            Self::Passive
...
pub struct InterAgentCommunication {
    ...
    /// Original transport identity when the provider-safe response item ID is normalized.
    #[serde(skip)]
    #[schemars(skip)]
    #[ts(skip)]
    ...
    pub content: String,
    /// Optional ingress-owned projection used only for model context. The canonical communication
    /// and its serialized/raw-event forms always retain `content` unchanged.
    #[serde(skip)]
    #[schemars(skip)]
    #[ts(skip)]
    pub model_visible_content: Option<String>,
    /// Presentation-only Cutex participant hints, excluded from model and rollout serialization.
    #[serde(skip)]
    #[schemars(skip)]
    #[ts(skip)]
    pub author_metadata: Option<Box<CutexParticipantPresentation>>,
    #[serde(skip)]
    #[schemars(skip)]
    #[ts(skip)]
    pub recipient_metadata: Option<Box<CutexParticipantPresentation>>,
    /// Typed, authenticated Task Service semantics for live presentation only.
    #[serde(skip)]
    #[schemars(skip)]
    #[ts(skip)]
    pub task_service_presentation: Option<Box<TaskServiceMessagePresentation>>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    #[ts(optional)]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    #[ts(optional)]
    pub delivery_mode: Option<InterAgentDeliveryMode>,
...
impl InterAgentCommunication {
    ...
    pub fn resolved_delivery_mode(&self) -> InterAgentDeliveryMode {
        self.delivery_mode
            .unwrap_or_else(|| InterAgentDeliveryMode::from_legacy_trigger_turn(self.trigger_turn))
    }
...
impl Op {
    pub fn kind(&self) -> &'static str {
        match self {
            ...
            Self::CutexUiActivity { .. } => "cutex_ui_activity",
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CutexUiActivityDisposition {
    Accepted,
    Duplicate,
    Stale,
}
...
pub enum EventMsg {
    ...
    /// Accepted Cutex presentation transport. This event is never persisted into rollout history.
    CutexUiActivity(CutexUiActivityEvent),
...
#[derive(Debug, Clone, Deserialize, Serialize, TS, JsonSchema)]
pub struct CutexUiActivityEvent {
    pub thread_id: ThreadId,
...
mod tests {
    ...
    #[test]
    fn inter_agent_delivery_modes_serialize_and_resolve_legacy_flags() {
        for (mode, wire_value, trigger_turn) in [
            (InterAgentDeliveryMode::AfterTurn, "after_turn", true),
            (InterAgentDeliveryMode::Soon, "soon", true),
            (InterAgentDeliveryMode::Passive, "passive", false),
            (InterAgentDeliveryMode::Interrupt, "interrupt", true),
        ] {
            assert_eq!(
                serde_json::to_value(mode).expect("serialize delivery mode"),
                json!(wire_value)
            );
            let communication = InterAgentCommunication::new_with_delivery_mode(
                AgentPath::root(),
            ...
            assert_eq!(communication.delivery_mode, Some(mode));
            assert_eq!(communication.trigger_turn, trigger_turn);
            assert_eq!(communication.resolved_delivery_mode(), mode);
        }

        for (trigger_turn, expected_mode) in [
            (false, InterAgentDeliveryMode::Passive),
            (true, InterAgentDeliveryMode::Soon),
        ] {
            ...
            assert_eq!(communication.delivery_mode, None);
            assert_eq!(communication.resolved_delivery_mode(), expected_mode);
        }
    }
