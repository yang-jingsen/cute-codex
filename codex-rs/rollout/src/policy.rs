...
pub fn should_persist_event_msg(ev: &EventMsg, history_mode: ThreadHistoryMode) -> bool {
    match ev {
        EventMsg::ItemCompleted(event)
            if event.turn_id == CUTEX_UI_ONLY_ACTIVITY_LANE_ID
                && matches!(
                    event.item,
                    TurnItem::ManagedAgentActivity(_)
                        | TurnItem::OutboundInterAgentMessage(_)
                        | TurnItem::TaskAssignmentActivity(_)
                        | TurnItem::TaskWatchdogActivity(_)
                ) =>
        {
            false
        }
        ...
        EventMsg::ItemStarted(event)
            if matches!(
                ...
                TurnItem::ManagedAgentActivity(_)
                    ...
                    | TurnItem::TaskAssignmentActivity(_)
                    | TurnItem::TaskWatchdogActivity(_)
            ) && event.turn_id != CUTEX_UI_ONLY_ACTIVITY_LANE_ID =>
        {
            true
        }
