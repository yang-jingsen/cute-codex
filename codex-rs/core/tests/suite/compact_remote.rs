...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn auto_remote_compact_failure_reports_error_and_preserves_after_turn_input() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    const MESSAGE_ID: &str = "after-turn-precompact-failure";
    const MESSAGE_TEXT: &str = "after_turn mail that triggers auto compact";
    ...
    let turn_complete = loop {
        ...
        match event.msg {
            ...
            EventMsg::ItemCompleted(ItemCompletedEvent {
                ...
                ..
            }) if item.id == MESSAGE_ID => recorded_mail = Some(item),
    ...
    assert!(
        error_messages
            .first()
            .is_some_and(|message| message.contains("Error running remote compact task")),
        "expected remote compact task error prefix, got {error_messages:?}"
    );
    assert_eq!(
        error_messages.len(),
        1,
        "the wrapper must not duplicate an inner compaction error"
    );
    let terminal_error = turn_complete
        .error
        .expect("compaction failure must mark TurnComplete as failed");
    assert_eq!(terminal_error.message, error_messages[0]);
    let recorded_mail =
        recorded_mail.expect("accepted after_turn mail must be recorded on failure");
    assert_eq!(recorded_mail.content, MESSAGE_TEXT);
    assert_eq!(
        recorded_mail.delivery_mode,
        InterAgentDeliveryMode::AfterTurn
    );
    assert_eq!(
        first_compact_mock.requests().len(),
        1,
        "expected one remote compact attempt before recording incoming mail"
    );
    assert!(
    ...
    insta::assert_snapshot!(
        ...
        format_labeled_requests_snapshot(
            "Remote pre-turn auto-compaction parse failure: compaction request excludes incoming after_turn mail; the failed turn records the accepted mail and stops.",
            &[(
                "Remote Compaction Request (Incoming after_turn Mail Excluded)",
