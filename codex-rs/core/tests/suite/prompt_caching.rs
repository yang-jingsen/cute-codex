...
async fn prompt_tools_are_consistent_across_requests(
    ...
    {
        expected_tools_names.push("cutex_task_service");
    }
    if std::env::var_os("CUTEX_AGENT_ID").is_some() {
        expected_tools_names.push("cutex_agent_management");
    }
    if std::env::var("CUTEX_AGENT_BUS_URL").is_ok_and(|value| !value.trim().is_empty()) {
        expected_tools_names.extend(["cutex_agent_list", "cutex_agent_send"]);
    }
