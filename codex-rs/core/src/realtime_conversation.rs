...
fn ensure_realtime_transport_allowed(force_http_transport: bool) -> CodexResult<()> {
    if force_http_transport {
        return Err(CodexErr::InvalidRequest(format!(
            "realtime transport is unavailable while {CUTE_CODEX_FORCE_HTTP_TRANSPORT_ENV_VAR} is enabled"
        )));
    }
    Ok(())
}
...
async fn prepare_realtime_start(
    ...
    ensure_realtime_transport_allowed(force_http_transport_enabled())?;
