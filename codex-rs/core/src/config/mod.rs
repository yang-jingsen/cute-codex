...
fn apply_notify_service_env_overrides(
    mut settings: NotifyServiceSettings,
    env: impl Fn(&str) -> Option<String>,
) -> NotifyServiceSettings {
    if let Some(url) = env("CODEX_NOTIFY_SERVICE_URL") {
        settings.notify_service_url = Some(url).filter(|value| !value.is_empty());
    }
    if let Some(token) = env("CODEX_NOTIFY_SERVICE_TOKEN") {
        settings.notify_service_token = Some(token).filter(|value| !value.is_empty());
    }
    if let Some(value) = env("CODEX_NOTIFY_IDLE_TIMEOUT")
        && let Ok(seconds) = value.parse::<u64>()
    {
        settings.notify_service_idle_timeout_secs = seconds;
    }
    if let Some(value) = env("CODEX_NOTIFY_COMPOSER_IDLE_TIMEOUT")
        && let Ok(seconds) = value.parse::<u64>()
    {
        settings.notify_service_composer_idle_timeout_secs = seconds;
    }
    if let Some(value) = env("CODEX_NOTIFY_APPROVAL_TIMEOUT")
        && let Ok(seconds) = value.parse::<u64>()
    {
        settings.notify_service_approval_timeout_secs = seconds;
    }
    if let Some(value) = env("CODEX_NOTIFY_STARTUP_IDLE_TIMEOUT")
        && let Ok(seconds) = value.parse::<u64>()
    {
        settings.notify_service_startup_idle_timeout_secs = seconds;
    }
    if let Some(name) = env("CODEX_NOTIFY_AGENT_NAME")
        && !name.is_empty()
    {
        settings.notify_service_agent_name = name;
    }
    if let Some(value) = env("CODEX_NOTIFY_MIN_TOKENS")
        && let Ok(tokens) = value.parse::<i64>()
    {
        settings.notify_service_min_tokens = tokens;
    }
    if let Some(value) = env("CODEX_NOTIFY_EVENTS") {
        settings.notify_service_events = parse_notify_service_events(&value);
    }
    if let Some(value) = env("CODEX_NOTIFY_USER_MESSAGE_CONTENT")
        && let Some(mode) = parse_notify_user_message_content(&value)
    {
        settings.notify_service_user_message_content = mode;
    }
    if let Some(value) = env("CODEX_NOTIFY_USER_MESSAGE_PREVIEW_CHARS")
        && let Ok(chars) = value.parse::<usize>()
    {
        settings.notify_service_user_message_preview_chars = chars;
    }
    settings
}
...
pub struct Config {
    ...
    /// Structured HTTP lifecycle notification settings for the TUI.
    pub notify_service: codex_config::types::NotifyServiceSettings,

    /// Presentation-only settings for typed Cutex timeline activity.
    pub tui_cutex_activity: TuiCutexActivitySettings,
...
pub async fn load_global_mcp_servers_with_overrides(
    codex_home: &Path,
    loader_overrides: LoaderOverrides,
) -> std::io::Result<BTreeMap<String, McpServerConfig>> {
        ...
        let config = Self {
            ...
            notify_service: {
                let ns = cfg
                    ...
                    .unwrap_or_default();
                apply_notify_service_env_overrides(ns, |key| std::env::var(key).ok())
            },
            // Cutex presentation is frontend-owned and loaded by the TUI from the shared
            // ~/.cutex/configs/tui.toml namespace. Core/product configuration must never
            // make these preferences authoritative or visible outside presentation.
