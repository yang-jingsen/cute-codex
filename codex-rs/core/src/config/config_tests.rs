...
#[test]
fn notify_service_env_overrides_apply_after_config_and_ignore_bad_values() {
    let values = HashMap::from([
        ("CODEX_NOTIFY_SERVICE_URL", "http://notify.test/push"),
        ("CODEX_NOTIFY_SERVICE_TOKEN", "token"),
        ("CODEX_NOTIFY_IDLE_TIMEOUT", "7"),
        ("CODEX_NOTIFY_COMPOSER_IDLE_TIMEOUT", "17"),
        ("CODEX_NOTIFY_APPROVAL_TIMEOUT", "27"),
        ("CODEX_NOTIFY_STARTUP_IDLE_TIMEOUT", "37"),
        ("CODEX_NOTIFY_AGENT_NAME", "agent-a"),
        ("CODEX_NOTIFY_MIN_TOKENS", "-3"),
        (
            "CODEX_NOTIFY_EVENTS",
            "session_started,turn_completed,unknown, hook_completed",
        ),
        ("CODEX_NOTIFY_USER_MESSAGE_CONTENT", "preview"),
        ("CODEX_NOTIFY_USER_MESSAGE_PREVIEW_CHARS", "19"),
    ]);
    let settings = apply_notify_service_env_overrides(
        NotifyServiceSettings {
            notify_service_url: Some("file-value".to_string()),
            notify_service_idle_timeout_secs: 1,
            ..NotifyServiceSettings::default()
        },
        |key| values.get(key).map(ToString::to_string),
    );

    assert_eq!(
        settings.notify_service_url.as_deref(),
        Some("http://notify.test/push")
    );
    assert_eq!(settings.notify_service_token.as_deref(), Some("token"));
    assert_eq!(settings.notify_service_idle_timeout_secs, 7);
    assert_eq!(settings.notify_service_composer_idle_timeout_secs, 17);
    assert_eq!(settings.notify_service_approval_timeout_secs, 27);
    assert_eq!(settings.notify_service_startup_idle_timeout_secs, 37);
    assert_eq!(settings.notify_service_agent_name, "agent-a");
    assert_eq!(settings.notify_service_min_tokens, -3);
    assert_eq!(
        settings.notify_service_events,
        vec![
    ...
    assert_eq!(
        settings.notify_service_user_message_content,
        NotifyServiceUserMessageContent::Preview
    );
    assert_eq!(settings.notify_service_user_message_preview_chars, 19);

    let invalid = HashMap::from([
        ("CODEX_NOTIFY_IDLE_TIMEOUT", "bad"),
        ("CODEX_NOTIFY_AGENT_NAME", ""),
        ("CODEX_NOTIFY_USER_MESSAGE_CONTENT", "bad"),
        ("CODEX_NOTIFY_USER_MESSAGE_PREVIEW_CHARS", "bad"),
    ]);
    let defaults = NotifyServiceSettings::default();
    let preserved = apply_notify_service_env_overrides(defaults.clone(), |key| {
        invalid.get(key).map(ToString::to_string)
    });
    assert_eq!(
        preserved.notify_service_idle_timeout_secs,
        defaults.notify_service_idle_timeout_secs
    );
    assert_eq!(
        preserved.notify_service_agent_name,
        defaults.notify_service_agent_name
    );
    assert_eq!(
        preserved.notify_service_user_message_content,
        defaults.notify_service_user_message_content
    );
    assert_eq!(
        preserved.notify_service_user_message_preview_chars,
        defaults.notify_service_user_message_preview_chars
    );
}
...
"#,
    ...
    assert!(err.to_string().contains(
        "model_providers.amazon-bedrock only supports changing `base_url`, `auth`, `http_headers`, `aws.profile`, `aws.region`, `aws.auth_refresh`, and `supports_agent_message_items`; other non-default provider fields are not supported"
    ));
}
...
#[tokio::test]
async fn legacy_product_cutex_activity_table_is_not_a_runtime_input() {
...
[tui.cutex_activity.managed_agent]
completed = "{agent} 已取餐"

[tui.cutex_activity.task_assignment]
attempt_started = "{assignee} accepted {task}"
"#;
    let parsed = toml::from_str::<ConfigToml>(cfg).expect("legacy table is tolerated");
    ...
    .await
    .expect("load customized TUI presentation");

    assert_eq!(
        resolved.tui_cutex_activity,
        TuiCutexActivitySettings::default()
    );
}
