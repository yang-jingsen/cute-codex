...
#[test]
fn cutex_rich_template_schema_exposes_strict_spans_and_named_or_rgb_colors() {
    let schema_json =
        codex_config::schema::cutex_tui_schema_json().expect("serialize Cutex TUI schema");
    let schema: serde_json::Value =
        serde_json::from_slice(&schema_json).expect("decode schema json");
    for presentation in [
        "TuiAgentManagementPhasePresentation",
        ...
        assert!(
            schema
                .pointer(&format!(
                    "/definitions/{presentation}/properties/rich_template"
                ))
                .is_some(),
            "{presentation} should expose rich_template"
        );
    }
    let color = schema
        .pointer("/definitions/TuiCutexForegroundColor/oneOf")
        .and_then(serde_json::Value::as_array)
        .expect("foreground schema should be named-or-RGB");
    assert!(color.iter().any(|branch| {
        branch
            .get("enum")
            .and_then(serde_json::Value::as_array)
            .is_some()
    ...
    assert!(
        color
            .iter()
            .any(|branch| branch.get("pattern") == Some(&serde_json::json!("^#[0-9A-Fa-f]{6}$")))
    );

    let fixture_path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../tui/cutex-tui.schema.json");
    let fixture: serde_json::Value = serde_json::from_slice(
        &std::fs::read(fixture_path).expect("read Cutex TUI schema fixture"),
    )
    .expect("decode Cutex TUI schema fixture");
    assert_eq!(canonicalize(&fixture), canonicalize(&schema));
}
