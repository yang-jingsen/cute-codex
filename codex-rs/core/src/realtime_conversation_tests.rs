...
use super::ensure_realtime_transport_allowed;
...
#[test]
fn forced_http_rejects_realtime_transport() {
    ensure_realtime_transport_allowed(/*force_http_transport*/ false)
        .expect("default transport selection should allow realtime");
    let error = ensure_realtime_transport_allowed(/*force_http_transport*/ true)
        .expect_err("forced HTTP should reject realtime");

    assert!(
        error
            .to_string()
            .contains("CUTE_CODEX_FORCE_HTTP_TRANSPORT")
    );
}
