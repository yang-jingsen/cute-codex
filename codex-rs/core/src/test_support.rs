...
/// Deterministic barrier for the inter-agent append/flush retry integration tests.
#[cfg(debug_assertions)]
pub fn inject_inter_agent_persistence_failures(
    thread: &CodexThread,
    live_thread: u64,
    append: u64,
    flush: u64,
    ...
    thread
        ...
        .inject_persistence_failures(live_thread, append, flush);
}

/// Rebuilds the persisted A4/A5 index exactly as resume does.
#[cfg(debug_assertions)]
pub async fn reconstructed_inter_agent_has_pending_model_actions(
    thread_id: ThreadId,
    items: &[codex_history::RolloutItem],
) -> bool {
    crate::inter_agent_delivery::InterAgentDeliveryTracker::from_rollout_items(thread_id, items)
