...
#[test]
fn force_http_transport_value_parses_truthy_and_falsey_values() {
    for value in ["1", " true ", "YES", "On"] {
        assert!(
            force_http_transport_value_enabled(value),
            "expected {value:?} to enable forced HTTP"
        );
    }
    for value in ["", "0", "false", "no", "off", "enabled"] {
        assert!(
            !force_http_transport_value_enabled(value),
            "expected {value:?} to leave transport selection unchanged"
        );
    }
}
