...
#[tokio::test]
async fn cutex_agent_bus_tools_follow_environment_gate() {
    let plan = probe(|_| {}).await;
    let tool_names = ["cutex_agent_list", "cutex_agent_send"];

    if cutex_agent_bus_available() {
        plan.assert_visible_contains(&tool_names);
        plan.assert_registered_contains(&tool_names);
    } else {
        plan.assert_visible_lacks(&tool_names);
        plan.assert_registered_lacks(&tool_names);
    }
}

#[tokio::test]
async fn ephemeral_system_threads_hide_cutex_runtime_tools() {
    let plan = probe(|turn| {
        update_config(turn, |config| {
            config.ephemeral = true;
        });
        turn.thread_source = Some(ThreadSource::Feature("system".to_string()));
    })
    .await;
...
async fn mcp_and_tool_search_follow_direct_and_deferred_tool_exposure() {
    ...
    .await;
    if TaskServiceDirectorHandler::from_environment().is_some() {
...
async fn excluded_deferred_namespaces_do_not_enable_nested_tool_guidance() {
    ...
    assert_eq!(
        ...
        TaskServiceDirectorHandler::from_environment().is_some()
    );
    plan.assert_registered_contains(&[
