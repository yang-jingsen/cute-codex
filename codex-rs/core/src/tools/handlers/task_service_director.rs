...
pub struct TaskServiceDirectorHandler {
...
impl TaskServiceDirectorHandler {
    ...
    async fn invoke_arguments(&self, arguments: &str) -> String {
        let args: DirectorArgs = match serde_json::from_str(arguments) {
            Ok(args) => args,
            Err(_) => return serialize_receipt(ModelReceipt::invalid_arguments()),
        };
        let request = match semantic_request(&args) {
            Ok(request) => request,
            Err(code) => {
                return serialize_receipt(ModelReceipt::no_write(&args, code));
            }
        };
        let request_body = match serde_json::to_vec(&request) {
            Ok(body) => body,
            Err(_) => {
                return serialize_receipt(ModelReceipt::no_write(
                    &args,
                    "invalid_semantic_payload",
                ));
            }
        };
        let receipt = match self.transport.post_director_action(request_body).await {
            Ok(response) => sanitize_provider_receipt(&args, response),
            Err(TransportError::ResponseUncertain) => ModelReceipt::response_uncertain(&args),
            Err(TransportError::Unavailable(code)) => ModelReceipt::no_write(&args, code),
            Err(TransportError::Rejected) => ModelReceipt::no_write(&args, "integration_rejected"),
            Err(TransportError::InvalidResponse) => {
                ModelReceipt::no_write(&args, "invalid_provider_response")
            }
        };
...
impl ToolExecutor<ToolInvocation> for TaskServiceDirectorHandler {
    fn tool_name(&self) -> ToolName {
        ToolName::plain(TASK_SERVICE_DIRECTOR_TOOL_NAME)
    }

    fn spec(&self) -> ToolSpec {
        create_task_service_director_tool()
    }
        ...
        Box::pin(async move {
            let ToolPayload::Function { arguments } = invocation.payload else {
                return Err(FunctionCallError::RespondToModel(
                    "cutex_task_service_director received an unsupported payload".to_string(),
                ));
            };
...
impl CoreToolRuntime for TaskServiceDirectorHandler {}
...
fn serialize_receipt(receipt: ModelReceipt) -> String {
    serde_json::to_string(&receipt).unwrap_or_else(|_| {
        format!(
            "{{\"schema\":\"{MODEL_RECEIPT_SCHEMA}\",\"status\":\"no_write\",\"operation\":\"invalid\",\"action_id\":\"unknown\",\"code\":\"serialization_failed\"}}"
        )
    })
}

#[cfg(test)]
#[path = "task_service_director_tests.rs"]
mod tests;
