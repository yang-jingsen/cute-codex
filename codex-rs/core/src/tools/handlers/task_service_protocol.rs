...
const WORKER_PREPARE_REQUEST_SCHEMA: &str = "cutex/task-service-worker-prepare/v2";
...
pub(super) fn prepare_request_body(action: &Value) -> Result<Vec<u8>, serde_json::Error> {
    serde_json::to_vec(&json!({
        "schema": WORKER_PREPARE_REQUEST_SCHEMA,
        "action": action,
    }))
}
