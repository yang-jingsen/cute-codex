...
use crate::tools::handlers::AgentManagementHandler;
...
use crate::tools::handlers::TaskServiceDirectorHandler;
use crate::tools::handlers::TaskServiceHandler;
...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    if cutex_runtime_tools_enabled(turn_context) {
        if let Some(handler) = TaskServiceHandler::from_environment() {
            registry.add(handler);
        }

        if let Some(handler) = TaskServiceDirectorHandler::from_environment() {
            registry.add(handler);
        }

        if let Some(handler) = AgentManagementHandler::from_environment() {
            registry.add(handler);
        }

        if cutex_agent_bus_available() {
            registry.add(CutexAgentListHandler);
            registry.add(CutexAgentSendHandler);
        }
    }
...
fn cutex_runtime_tools_enabled(turn_context: &TurnContext) -> bool {
    !(turn_context.config.ephemeral
        && matches!(
            turn_context.thread_source.as_ref(),
