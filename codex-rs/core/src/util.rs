...
/// Process launch override that forces Responses requests onto HTTP.
...
/// Returns whether the process launch environment requests HTTP-only transport.
pub fn force_http_transport_enabled() -> bool {
    std::env::var(CUTE_CODEX_FORCE_HTTP_TRANSPORT_ENV_VAR)
        .is_ok_and(|value| force_http_transport_value_enabled(&value))
}

fn force_http_transport_value_enabled(value: &str) -> bool {
    matches!(
