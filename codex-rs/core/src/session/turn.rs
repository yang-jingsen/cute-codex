...
/// Ensure a failed compaction is terminal on the wire without duplicating an
/// affecting error already emitted by the concrete compaction implementation.
///
/// TODO(cute-codex): remove this compatibility guard when upstream resolves
/// openai/codex#25619 with equivalent error-status behavior.
async fn ensure_compaction_failure_reported(
    sess: &Session,
    turn_context: &TurnContext,
    err: &CodexErr,
) {
    let protocol_error = err.to_codex_protocol_error();
    sess.emit_turn_error_lifecycle(turn_context, protocol_error)
        .await;
    ...
    let mut event = err.to_error_event(Some("Compaction failed".to_string()));
    if !event.affects_turn_status() {
        event.codex_error_info = Some(CodexErrorInfo::Other);
    }
...
pub(crate) async fn run_turn(
    ...
    .await
    {
        ...
        ensure_compaction_failure_reported(&sess, &turn_context, &err).await;
        // Pre-turn compaction deliberately excludes incoming items. Preserve
        // accepted input on failure so a controller retry can continue from
        // history without an automatic compact/fail/wake loop.
        run_hooks_and_record_inputs(&sess, &turn_context, &input, PersistContext::Standard).await;
        error!(error = %err, "Failed to run pre-sampling compact");
        return Ok(None);
    }
    ...
    loop {
        ...
        let sampling_request_result: CodexResult<_> = async {
            ...
            let claimed_model_actions = if sess
                .input_queue
                .accepts_mailbox_delivery_for_current_turn(&sess.active_turn, &turn_context.sub_id)
                .await
            {
                sess.inter_agent_delivery_tracker
                    .claim_model_actions_for_input(&sampling_request_input, &turn_context.sub_id)
                    .await
            } else {
                Vec::new()
            };
            if let Err(err) = sess
                .persist_inter_agent_model_actions(&claimed_model_actions)
                .await
            {
                sess.inter_agent_delivery_tracker
                    .restore_model_actions(&claimed_model_actions)
                    .await;
                return Err(CodexErr::Io(std::io::Error::other(format!(
                    "failed to durably reserve actionable A4 work for a model request: {err}"
                ))));
            }
            ...
            .await;
            match result {
                Ok((output, input)) => Ok((output, input, claimed_model_actions)),
                Err(err) => {
                    sess.inter_agent_delivery_tracker
                        .restore_model_actions(&claimed_model_actions)
                        .await;
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input, claimed_model_actions)) => {
                ...
                if had_output {
                    if let Err(err) = sess
                        .persist_inter_agent_model_action_completions(&claimed_model_actions)
                        .await
                    {
                        sess.inter_agent_delivery_tracker
                            .restore_model_actions(&claimed_model_actions)
                            .await;
                        return Err(CodexErr::Io(std::io::Error::other(format!(
                            "failed to durably complete actionable A5 work: {err}"
                        ))));
                    }
                    sess.inter_agent_delivery_tracker
                        .complete_model_actions(&claimed_model_actions)
                        .await;
                } else {
                    // A request with no model output is not A5. Preserve the obligation so the
                    // turn cannot finish successfully while actionable A4 work is stranded.
                    sess.inter_agent_delivery_tracker
                        .restore_model_actions(&claimed_model_actions)
                        .await;
                }
                ...
                if should_roll_over {
                    ...
                    .await
                    {
                        ...
                        ensure_compaction_failure_reported(&sess, &turn_context, &err).await;
                        error!(error = %err, "Failed to run mid-turn compact");
                        return Ok(None);
                    }
                    if run_pending_session_start_hooks(&sess, &turn_context).await {
                        return Ok(None);
