use super::cutex_ui_activity::CutexUiActivityTracker;
...
pub(crate) struct Session {
    ...
    pub(crate) inter_agent_delivery_tracker: InterAgentDeliveryTracker,
    pub(super) inter_agent_delivery_attempts:
        Mutex<HashMap<String, Arc<InterAgentPersistenceAttempt>>>,
    pub(super) cutex_ui_activity_tracker: Mutex<CutexUiActivityTracker>,
...
pub(super) struct InterAgentPersistenceAttempt {
    pub(super) turn_context: Arc<TurnContext>,
    pub(super) items: Vec<ResponseItem>,
    pub(super) canonical_item: TurnItem,
    pub(super) receipt: codex_history::InterAgentDeliveryReceipt,
    pub(super) rollout_items: Vec<RolloutItem>,
    pub(super) actionable: bool,
    pub(super) phase: Mutex<InterAgentPersistencePhase>,
    pub(super) retry_running: AtomicBool,
    pub(super) finalized: AtomicBool,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum InterAgentPersistencePhase {
    NeedsAppend { verify_absent: bool },
    NeedsFlush,
}

pub(super) enum InterAgentPersistenceError {
    Retryable(String),
    Conflict(String),
}
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let cutex_ui_activity_tracker =
                CutexUiActivityTracker::from_rollout_items(initial_history.get_rollout_items());
            let inter_agent_delivery_tracker = InterAgentDeliveryTracker::from_rollout_items(
                thread_id,
            ...
            let sess = Arc::new(Session {
                ...
                inter_agent_delivery_attempts: Mutex::new(HashMap::new()),
                cutex_ui_activity_tracker: Mutex::new(cutex_ui_activity_tracker),
