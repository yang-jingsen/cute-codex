...
pub async fn inter_agent_communication(
    ...
    match sess
        ...
        .await
    {
        crate::inter_agent_delivery::AdmissionDecision::Admit => {}
        crate::inter_agent_delivery::AdmissionDecision::Retry => {
            sess.retry_inter_agent_delivery(&communication).await;
            return;
        }
        crate::inter_agent_delivery::AdmissionDecision::Duplicate
        | crate::inter_agent_delivery::AdmissionDecision::Conflict => return,
    }
    let delivery_mode = communication.resolved_delivery_mode();
    communication.trigger_turn = delivery_mode.trigger_turn();
    ...
    if !accepted {
        sess.inter_agent_delivery_tracker
            .cancel_admission(&communication)
            .await;
        return;
    }
    sess.inter_agent_delivery_tracker
        .mark_pending(&communication)
        .await;
    crate::agent_communication::emit_agent_communication_receive(&sub_id);
    if delivery_mode == codex_protocol::protocol::InterAgentDeliveryMode::Interrupt {
        sess.interrupt_task().await;
    }
...
/// Emits one UI-only Cutex activity without recording any model-visible item.
///
/// The rollout policy keeps authoritative native-turn activity durable and treats the reserved
/// Cutex lane as live presentation transport only.
pub async fn cutex_ui_activity(
    sess: &Arc<Session>,
    turn_id: String,
    delivery: CutexUiActivityDelivery,
    activity: CutexUiActivity,
) -> CodexResult<CutexUiActivityDisposition> {
    let disposition = sess
        .cutex_ui_activity_tracker
        .lock()
        .await
        .register(&activity);
    if disposition != CutexUiActivityDisposition::Accepted {
        return Ok(disposition);
    }

    let item = activity.into_turn_item();
    let msg = EventMsg::CutexUiActivity(CutexUiActivityEvent {
        thread_id: sess.thread_id(),
        delivery,
        item,
    });
    sess.send_event_raw(Event { id: turn_id, msg }).await;
    Ok(disposition)
}
        ...
        let should_exit = async {
            match sub.op {
                ...
                Op::CutexUiActivity {
                    ...
                    let result = cutex_ui_activity(&sess, turn_id, delivery, activity).await;
                    let _ = reply.send(result);
                    false
                }
                Op::ExecApproval {
