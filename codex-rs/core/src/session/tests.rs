...
use crate::inter_agent_delivery::InterAgentDeliveryQuery;
use crate::inter_agent_delivery::InterAgentDeliveryState;
use crate::inter_agent_delivery::InterAgentDeliveryTracker;
use crate::inter_agent_delivery::inter_agent_semantic_sha256;
...
fn cutex_delivery(
    sequence: u64,
    recovered: bool,
) -> codex_protocol::items::CutexUiActivityDelivery {
...
#[tokio::test]
async fn cutex_ui_activity_lane_emits_lifecycle_without_mutating_model_context() {
    ...
    let disposition = handlers::cutex_ui_activity(
        &session,
        CUTEX_UI_ONLY_ACTIVITY_LANE_ID.to_string(),
        cutex_delivery(1, false),
        CutexUiActivity::ManagedAgentActivity(Box::new(ManagedAgentActivityItem {
            ...
            phase_event_id: Some("agent-management:action-1:phase:9".to_string()),
            phase: Some(AgentManagementPhase::Complete),
            ...
            initial_task_preview: Some("implement the focused task".to_string()),
            detail: None,
            runtime_generation: Some(3),
        })),
    )
    .await
    .expect("UI-only activity should be accepted");

    assert_eq!(disposition, CutexUiActivityDisposition::Accepted);
    let event = rx.recv().await.expect("UI lifecycle event");
    let EventMsg::CutexUiActivity(presented) = event.msg else {
        panic!("expected UI-only Cutex delivery")
    };
    assert_eq!(event.id, CUTEX_UI_ONLY_ACTIVITY_LANE_ID);
    assert_eq!(presented.delivery, cutex_delivery(1, false));
    assert!(matches!(
        presented.item,
    ...
    let after = raw_history_items(&session.clone_history().await);
    assert_eq!(
        after, before,
        "EventMsg-only ingestion must not enter ContextManager"
    );
}

#[tokio::test]
async fn first_stale_watchdog_is_presentation_only_and_preserves_model_context() {
    use codex_protocol::items::CUTEX_UI_ONLY_ACTIVITY_LANE_ID;
    use codex_protocol::items::CutexUiActivity;
    ...
    let (session, _turn_context, rx) = make_session_and_context_with_rx().await;
    let before = raw_history_items(&session.clone_history().await);
    handlers::cutex_ui_activity(
        &session,
        CUTEX_UI_ONLY_ACTIVITY_LANE_ID.to_string(),
        cutex_delivery(10, false),
        CutexUiActivity::TaskWatchdogActivity(TaskWatchdogActivityItem {
            id: "episode-1".into(),
            event_id: "fact-1".into(),
    ...
    .await
    .expect("watchdog fact accepted");
    let event = rx.recv().await.expect("presentation lifecycle");
    let EventMsg::CutexUiActivity(presented) = event.msg else {
        panic!("watchdog card remains on the UI-only Cutex lane")
    };
    assert_eq!(presented.delivery, cutex_delivery(10, false));
    assert!(matches!(presented.item, TurnItem::TaskWatchdogActivity(_)));
    assert_eq!(raw_history_items(&session.clone_history().await), before);
}
...
#[tokio::test]
async fn identified_plaintext_inter_agent_message_emits_and_persists_canonical_item() {
    ...
    let author = AgentPath::root().join("worker").expect("author path");
    let recipient = AgentPath::root();
    let other_recipient = AgentPath::root().join("reviewer").expect("recipient path");
    let message_id = ResponseItemId::from_server("external-message-1".to_string());
    let mut communication = InterAgentCommunication::new_with_delivery_mode(
        author.clone(),
        recipient.clone(),
        vec![other_recipient.clone()],
    ...
    let started = rx.recv().await.expect("started inter-agent item event");
    let EventMsg::ItemStarted(ItemStartedEvent {
        item: TurnItem::InterAgentMessage(started_item),
        ..
    }) = started.msg
    else {
        panic!("expected inter-agent item started event");
    };
    let expected_item = InterAgentMessageItem {
        id: message_id.to_string(),
        author,
        recipient,
        other_recipients: vec![other_recipient],
    ...
    let completed = rx.recv().await.expect("completed inter-agent item event");
    let EventMsg::ItemCompleted(ItemCompletedEvent {
        item: TurnItem::InterAgentMessage(completed_item),
        ..
    }) = completed.msg
    else {
        panic!("expected inter-agent item completed event");
    };
    assert_eq!(completed_item, expected_item);
    assert!(rx.try_recv().is_err(), "no parallel legacy event expected");
    ...
    let persisted_items = resumed
        ...
        .filter_map(|item| match item {
            RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
                item: TurnItem::InterAgentMessage(item),
                ..
            })) => Some(item.clone()),
            _ => None,
        })
        .collect::<Vec<_>>();
    assert_eq!(persisted_items, vec![expected_item]);
}

#[tokio::test]
async fn inter_agent_a4_retries_live_thread_append_and_flush_failures_exactly_once() {
    for (case, live_thread_failures, append_failures, flush_failures) in [
        ("live_thread", 3, 0, 0),
        ("append", 0, 3, 0),
        ("flush", 0, 0, 3),
    ] {
        ...
        session
            ...
            .inject_persistence_failures(live_thread_failures, append_failures, flush_failures);
        let message_id = format!("retry-{case}-1");
        let mut communication = InterAgentCommunication::new_with_delivery_mode(
            AgentPath::root().join("sender").expect("author path"),
            AgentPath::root().join("recipient").expect("recipient path"),
            vec![AgentPath::root().join("observer").expect("observer path")],
        ...
        communication.external_message_id = Some(message_id.clone());
        ...
        session
            .record_inter_agent_communication(&turn_context, communication.clone())
            .await;
        assert_eq!(
            session
                .inter_agent_delivery_tracker
                .statuses(std::slice::from_ref(&query))
                .await[0]
                .state,
            InterAgentDeliveryState::RetryableError,
            "{case}: the injected failure must not publish A4"
        );
        assert!(
            rx.try_recv().is_err(),
            "{case}: lifecycle must wait for durable A4"
        );

        let duplicate_deliveries = (0..8).map(|ordinal| {
        ...
        for duplicate in duplicate_deliveries {
            duplicate.await.expect("duplicate retry task");
        }
        let mut changed = communication.clone();
        ...
        .await;
        let changed_query = InterAgentDeliveryQuery {
            message_id: message_id.clone(),
        ...
        assert_eq!(
            session
                .inter_agent_delivery_tracker
                .statuses(&[changed_query])
                .await[0]
                .state,
            InterAgentDeliveryState::Conflict,
            "{case}: changed semantics must conflict during recovery"
        );

        let receipt = timeout(StdDuration::from_secs(5), async {
            loop {
                let state = session
                    .inter_agent_delivery_tracker
                    .statuses(std::slice::from_ref(&query))
                    ...
                    .state
                    .clone();
                if let InterAgentDeliveryState::ContextPersisted(receipt) = state {
                    break receipt;
                }
        ...
        .await
        .unwrap_or_else(|_| panic!("{case}: retry did not reach A4"));
        ...
        assert!(matches!(
            timeout(StdDuration::from_secs(1), rx.recv())
                ...
                .msg,
            EventMsg::ItemStarted(ItemStartedEvent {
                item: TurnItem::InterAgentMessage(_),
                ..
            })
        ));
        assert!(matches!(
        ...
        assert!(rx.try_recv().is_err(), "{case}: duplicate lifecycle event");
        assert_eq!(
            raw_history_items(&session.clone_history().await)
                .iter()
                .filter(|item| item.id().is_some_and(|id| id.as_str() == message_id))
                .count(),
            1,
            "{case}: duplicate model-visible history item"
        );
            ...
            resumed
                ...
                .filter(|item| matches!(
                    ...
                    RolloutItem::InterAgentCommunicationMetadata {
                        delivery_receipt: Some(persisted),
                        ..
                    } if persisted.message_id == message_id
                ))
                .count(),
            1,
            "{case}: duplicate durable receipt marker"
        );
        assert_eq!(
            resumed
                .history
                ...
                .filter(|item| matches!(
                    ...
                    RolloutItem::ResponseItem(item)
                        if item.id().is_some_and(|id| id.as_str() == message_id)
                ))
                .count(),
            1,
            "{case}: duplicate durable response item"
        );
        ...
        assert_eq!(
            rebuilt.statuses(&[query]).await[0].state,
            InterAgentDeliveryState::ContextPersisted(receipt),
            "{case}: restart must reconstruct the exact final receipt"
        );
    }
}

#[tokio::test]
async fn private_or_unidentified_inter_agent_messages_do_not_emit_canonical_items() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let unidentified = InterAgentCommunication::new(
    ...
    session
        .record_inter_agent_communication(&turn_context, unidentified)
        .await;
    assert!(matches!(
        rx.recv().await.expect("unidentified raw response").msg,
        EventMsg::RawResponseItem(_)
    ));
    assert!(rx.try_recv().is_err(), "unidentified mail is not canonical");
    ...
    encrypted.id = Some(ResponseItemId::from_server("private-message-1".to_string()));

    session
        .record_inter_agent_communication(&turn_context, encrypted)
        .await;
    assert!(matches!(
        rx.recv().await.expect("encrypted raw response").msg,
        EventMsg::RawResponseItem(_)
    ));
    assert!(rx.try_recv().is_err(), "encrypted mail is not canonical");
}
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let session = Session {
        ...
        inter_agent_delivery_tracker:
            crate::inter_agent_delivery::InterAgentDeliveryTracker::from_rollout_items(
                thread_id,
                &[],
            ),
        inter_agent_delivery_attempts: Mutex::new(std::collections::HashMap::new()),
        cutex_ui_activity_tracker: Mutex::new(Default::default()),
