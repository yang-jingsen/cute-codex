...
#[test]
fn forced_http_disables_responses_websocket_without_latching_fallback() {
    ...
    assert!(client.responses_websocket_enabled_with_force_http(false));
    assert!(!client.responses_websocket_enabled_with_force_http(true));
    assert!(client.responses_websocket_enabled_with_force_http(false));
}

#[test]
fn response_request_preparation_strips_provider_unsafe_existing_history_ids() {
    const OVERSIZED_AGENT_MESSAGE_ID: &str =
        "amsg_tsc_tsn-0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
    const VALID_AGENT_MESSAGE_ID: &str = "amsg_existing_history_1";

    let client = test_model_client(SessionSource::Cli);
    let existing_history_message = |id: Option<ResponseItemId>| ResponseItem::AgentMessage {
        id,
        author: "/root/task-service".to_string(),
        recipient: "/root".to_string(),
        content: vec![AgentMessageInputContent::InputText {
            text: "existing rollout message".to_string(),
        }],
        internal_chat_message_metadata_passthrough: None,
    };
    let mut input = vec![
        existing_history_message(Some(ResponseItemId::from_server(
            OVERSIZED_AGENT_MESSAGE_ID.to_string(),
        ))),
        existing_history_message(Some(ResponseItemId::from_server(
            VALID_AGENT_MESSAGE_ID.to_string(),
        ))),
        existing_history_message(Some(ResponseItemId::from_server(
            "legacy-unprefixed".to_string(),
        ))),
    ];
...
#[tokio::test]
async fn compact_request_strips_unsafe_agent_message_id_from_existing_history() -> anyhow::Result<()>
{
    const OVERSIZED_AGENT_MESSAGE_ID: &str =
        "amsg_tsc_tsn-0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
    ...
    provider.supports_agent_message_items = Some(true);
    let client = test_model_client_with_provider(ThreadId::new(), SessionSource::Cli, provider);
    let prompt = Prompt {
        input: vec![ResponseItem::AgentMessage {
            ...
            recipient: "/root".to_string(),
            content: vec![AgentMessageInputContent::InputText {
                text: "polluted compact history fixture".to_string(),
            }],
            internal_chat_message_metadata_passthrough: None,
        }],
    ...
    let requests = server
        .received_requests()
        .await
        .expect("server should record compact request");
    let request = requests
        .iter()
        .find(|request| request.url.path() == "/v1/responses/compact")
        .expect("compact request should be captured");
    let body: serde_json::Value = serde_json::from_slice(&request.body)?;
    let agent_message = body["input"]
        .as_array()
        .and_then(|input| input.iter().find(|item| item["type"] == "agent_message"))
        .expect("existing AgentMessage should remain in compact input");
    assert!(agent_message.get("id").is_none());
    assert!(!body.to_string().contains(OVERSIZED_AGENT_MESSAGE_ID));
...
#[test]
fn native_agent_message_input_is_preserved() {
    ...
    let request = build_test_responses_request(&client, &prompt).expect("request should build");

    assert_eq!(request.input, vec![agent_message]);
    assert_eq!(prompt.input, request.input);
}

#[test]
fn custom_provider_projects_plaintext_agent_message_without_mutating_prompt() {
    let client = test_model_client(SessionSource::Cli);
    let ordinary_message = ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![ContentItem::InputText {
            text: "ordinary input".to_string(),
        }],
        phase: None,
        internal_chat_message_metadata_passthrough: None,
    };
    ...
    let function_call = ResponseItem::FunctionCall {
        ...
        namespace: None,
        arguments: "{}".to_string(),
        encrypted_function_args: Some(vec!["encrypted-args".to_string()]),
        call_id: "call-1".to_string(),
        internal_chat_message_metadata_passthrough: None,
    };
    let original_input = vec![
        ordinary_message.clone(),
        agent_message.clone(),
        function_call,
    ];
    let prompt = Prompt {
    ...
    assert_eq!(prompt.input[1], agent_message);
}

#[test]
fn custom_provider_rejects_non_plaintext_agent_message_without_exposing_content() {
    let cases = [
        vec![AgentMessageInputContent::EncryptedContent {
            encrypted_content: "encrypted-only-secret".to_string(),
        }],
        vec![
            AgentMessageInputContent::InputText {
            ...
            AgentMessageInputContent::EncryptedContent {
                encrypted_content: "mixed-secret".to_string(),
            },
        ],
    ...
    for content in cases {
        ...
        let error = build_test_responses_request(&client, &prompt)
            .expect_err("unsupported agent message should fail");

        assert_eq!(
            error.to_string(),
            super::UNSUPPORTED_AGENT_MESSAGE_INPUT_ERROR
        );
