...
const MAX_PROVIDER_RESPONSE_ITEM_ID_BYTES: usize = 64;

fn response_item_id_is_provider_safe(id: &ResponseItemId) -> bool {
    id.is_prefixed() && id.len() <= MAX_PROVIDER_RESPONSE_ITEM_ID_BYTES
}
...
const UNSUPPORTED_AGENT_MESSAGE_INPUT_ERROR: &str = "active model provider does not support agent_message input items; incoming agent messages must contain only nonblank plaintext";
...
impl ModelClient {
            ...
            .supports_agent_message_items
        {
            for item in &mut input {
                ...
                let text = plaintext_agent_message_content(content).ok_or_else(|| {
                    CodexErr::InvalidRequest(UNSUPPORTED_AGENT_MESSAGE_INPUT_ERROR.to_string())
                })?;
    ...
    fn prepare_response_items_for_request(&self, input: &mut [ResponseItem]) {
        for item in input {
            if item
                .id()
                .is_some_and(|id| !response_item_id_is_provider_safe(id))
            {
    ...
    pub fn responses_websocket_enabled(&self) -> bool {
        self.responses_websocket_enabled_with_force_http(force_http_transport_enabled())
    }

    fn responses_websocket_enabled_with_force_http(&self, force_http_transport: bool) -> bool {
        if force_http_transport
