...
"#,
    ...
    #[test]
    fn tui_flattens_notify_service_settings() {
        let config: ConfigToml = toml::from_str(
            r#"
[tui]
notify_service_url = "http://notify.test/push"
notify_service_idle_timeout_secs = 9
notify_service_events = ["session_started", "turn_completed"]
notify_service_user_message_content = "full"
"#,
        )
        .expect("notify service settings should deserialize under tui");
        let settings = config.tui.expect("tui config").notify_service_settings;

        assert_eq!(
            settings.notify_service_url.as_deref(),
            Some("http://notify.test/push")
        );
        assert_eq!(settings.notify_service_idle_timeout_secs, 9);
        assert_eq!(
            settings.notify_service_events,
            vec![
                crate::types::NotifyServiceEvent::SessionStarted,
                crate::types::NotifyServiceEvent::TurnCompleted,
            ]
        );
        assert_eq!(
            settings.notify_service_user_message_content,
            crate::types::NotifyServiceUserMessageContent::Full
        );
    }
}
