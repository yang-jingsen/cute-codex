...
use crate::user_config_path::user_config_path_from_value;
...
impl LoaderOverrides {
    ...
    pub fn user_config_path(&self, codex_home: &Path) -> std::io::Result<AbsolutePathBuf> {
        let value = std::env::var(crate::CODEX_CONFIG_FILE_ENV).ok();
        self.user_config_path_with_env(codex_home, value.as_deref())
    }

    fn user_config_path_with_env(
        &self,
        codex_home: &Path,
        value: Option<&str>,
        ...
        match self.user_config_path.as_ref() {
            ...
            None => Ok(
                user_config_path_from_value(codex_home, value).unwrap_or_else(|| {
                    AbsolutePathBuf::resolve_path_against_base(crate::CONFIG_TOML_FILE, codex_home)
                }),
            ),
        }
    }
