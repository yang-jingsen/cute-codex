...
/// Build the schema for the frontend-owned shared Cutex TUI configuration.
pub fn cutex_tui_schema() -> RootSchema {
    SchemaSettings::draft07()
        .with(|settings| {
            settings.option_add_null_type = false;
        })
        .into_generator()
        .into_root_schema_for::<TuiCutexActivitySettings>()
}
...
/// Render the shared Cutex TUI schema as pretty-printed canonical JSON.
pub fn cutex_tui_schema_json() -> anyhow::Result<Vec<u8>> {
    let value = serde_json::to_value(cutex_tui_schema())?;
    let value = canonicalize(&value);
...
/// Write the frontend-owned shared Cutex TUI schema fixture.
pub fn write_cutex_tui_schema(out_path: &Path) -> anyhow::Result<()> {
    std::fs::write(out_path, cutex_tui_schema_json()?)?;
    Ok(())
}
