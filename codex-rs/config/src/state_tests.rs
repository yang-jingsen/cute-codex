...
#[test]
fn explicit_user_config_path_wins_over_environment_value() {
    let temp_dir = TempDir::new().expect("tempdir");
    let explicit = test_user_config_path(&temp_dir, "explicit.toml");
    let overrides = LoaderOverrides {
        user_config_path: Some(explicit.clone()),
        ..Default::default()
    };

    let resolved = overrides
        .user_config_path_with_env(temp_dir.path(), Some("environment.toml"))
        .expect("resolve user config path");
