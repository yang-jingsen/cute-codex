...
#[test]
fn inter_agent_model_action_completion_is_explicit_and_additive() -> Result<()> {
    let legacy_schedule = json!({
        ...
        "payload": {
            "trigger_turn": false,
            "model_action": {
                "message_id": "amsg_restart_1",
                "semantic_sha256": "a".repeat(64),
                "turn_id": "turn-a5",
            },
        },
    });
    let schedule = serde_json::from_value::<RolloutLine>(legacy_schedule)?;
    let RolloutItem::InterAgentCommunicationMetadata {
        model_action: Some(action),
        ..
    } = schedule.item
    else {
        panic!("expected model action schedule");
    };
    assert!(
        !action.completed,
        "old schedules must never imply completion"
    );

    let completion = RolloutLine {
        timestamp: "2026-08-30T00:00:01.000Z".to_string(),
        ordinal: None,
        item: RolloutItem::InterAgentCommunicationMetadata {
            trigger_turn: false,
            delivery_receipt: None,
            model_action: Some(InterAgentModelAction {
                completed: true,
                ..action
            }),
        },
    ...
    assert_eq!(serialized["payload"]["model_action"]["completed"], true);
    let restored = serde_json::from_value::<RolloutLine>(serialized)?;
    let RolloutItem::InterAgentCommunicationMetadata {
        model_action: Some(restored_action),
        ..
    } = restored.item
    else {
        panic!("expected explicit completion action");
    };
    assert!(restored_action.completed);
    assert_eq!(restored_action.message_id, "amsg_restart_1");
    assert_eq!(restored_action.turn_id, "turn-a5");
    Ok(())
}
