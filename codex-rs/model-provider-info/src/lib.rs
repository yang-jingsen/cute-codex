...
pub struct ModelProviderInfo {
    ...
    /// Whether this provider accepts `agent_message` items in Responses API input.
    /// When unset, official OpenAI providers enable support and other providers disable it.
    pub supports_agent_message_items: Option<bool>,
}
...
impl ModelProviderInfo {
    ...
    pub fn create_openai_provider(base_url: Option<String>) -> ModelProviderInfo {
        ModelProviderInfo {
            ...
            supports_agent_message_items: None,
        }
    }
...
pub fn merge_configured_model_providers(
    ...
    for (key, mut provider) in configured_model_providers {
        if matches!(
            ...
            let supports_agent_message_items_override =
                provider.supports_agent_message_items.take();
            if provider != ModelProviderInfo::default() {
                return Err(format!(
                    "model_providers.{key} only supports changing \
`base_url`, `auth`, `http_headers`, `aws.profile`, `aws.region`, `aws.auth_refresh`, and \
...
other non-default provider fields are not supported"
            ...
            if let Some(built_in_provider) = model_providers.get_mut(&key) {
                ...
                if let Some(supports_agent_message_items_override) =
                    supports_agent_message_items_override
                {
                    built_in_provider.supports_agent_message_items =
                        Some(supports_agent_message_items_override);
                }
            }
        } else {
