...
#[test]
fn test_agent_message_capability_round_trips() {
    let provider: ModelProviderInfo = toml::from_str(
        r#"
name = "Custom"
supports_agent_message_items = true
"#,
    )
    .expect("provider should deserialize");

    assert_eq!(provider.supports_agent_message_items, Some(true));
    let serialized = toml::to_string(&provider).expect("provider should serialize");
    assert!(serialized.contains("supports_agent_message_items = true"));
}
...
fn test_merge_configured_model_providers_applies_amazon_bedrock_transport_overrides() {
    ...
    let configured_model_providers = std::collections::HashMap::from([(
        ...
        ModelProviderInfo {
            ...
            supports_agent_message_items: Some(true),
            ..ModelProviderInfo::default()
        },
    )]);
    ...
    expected_provider.supports_agent_message_items = Some(true);
