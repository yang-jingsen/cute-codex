...
impl MessageProcessor {
    ...
    pub(crate) async fn connection_closed(
        ...
        // Connection-owned processes must be terminated before draining RPCs.
        // An active command/exec request holds the gate open until its child
        // exits, so waiting first would delay disconnect cleanup by the child's
        // timeout and could leave host resources alive after the peer is gone.
    ...
    async fn handle_initialized_client_request(
        ...
        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match codex_request {
            ...
            ClientRequest::ThreadInterAgentMessage { params, .. } => {
                self.turn_processor
                    .thread_inter_agent_message(&request_id, params)
                    .await
            }
            ClientRequest::ThreadInterAgentMessageStatus { params, .. } => {
                self.turn_processor
                    .thread_inter_agent_message_status(&request_id, params)
                    .await
            }
            ClientRequest::ThreadCutexActivity { params, .. } => {
                self.turn_processor
                    .thread_cutex_activity(&request_id, params)
                    .await
            }
