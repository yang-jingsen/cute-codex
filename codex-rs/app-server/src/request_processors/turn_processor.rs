use super::external_message_id::normalize_external_message_id;
use super::thread_input::ensure_direct_input_allowed;
use super::*;
use codex_agent_extension::AgentInvocation;
use codex_agent_extension::AgentRun;
use codex_agent_extension::AgentRunner;
use codex_core::inter_agent_delivery::InterAgentDeliveryQuery as CoreInterAgentDeliveryQuery;
use codex_core::inter_agent_delivery::InterAgentDeliveryState as CoreInterAgentDeliveryState;
...
impl TurnRequestProcessor {
    ...
    pub(crate) async fn thread_inter_agent_message(
        ...
        self.thread_inter_agent_message_response_inner(request_id, params)
            .await
            .map(|response| Some(response.into()))
    }

    pub(crate) async fn thread_inter_agent_message_status(
        &self,
        request_id: &ConnectionRequestId,
        params: ThreadInterAgentMessageStatusParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.thread_inter_agent_message_status_response_inner(request_id, params)
            .await
            .map(|response| Some(response.into()))
    }

    pub(crate) async fn thread_cutex_activity(
        &self,
        request_id: &ConnectionRequestId,
        params: ThreadCutexActivityParams,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        self.thread_cutex_activity_response_inner(request_id, params)
            .await
            .map(|response| Some(response.into()))
    }
    ...
    async fn thread_inter_agent_message_response_inner(
        &self,
        request_id: &ConnectionRequestId,
        mut params: ThreadInterAgentMessageParams,
    ) -> Result<ThreadInterAgentMessageResponse, JSONRPCErrorError> {
        let external_message_id = params.message_id.clone();
        params.message_id =
            normalize_external_message_id(&params.message_id).map_err(invalid_request)?;
        let mut communication = params.to_core_communication().map_err(invalid_request)?;
        communication.external_message_id = Some(external_message_id);
        let (_, thread) = self.load_thread(&params.thread_id).await?;
        let submission_id = self
            ...
            .map_err(|err| match err.details() {
                CodexErrorDetails::InvalidRequest(message) => invalid_request(message.clone()),
                _ => internal_error(format!("failed to submit inter-agent message: {err}")),
            })?;
        Ok(ThreadInterAgentMessageResponse { submission_id })
    }

    async fn thread_inter_agent_message_status_response_inner(
        &self,
        _request_id: &ConnectionRequestId,
        params: ThreadInterAgentMessageStatusParams,
    ) -> Result<ThreadInterAgentMessageStatusResponse, JSONRPCErrorError> {
        const MAX_MESSAGES: usize = 256;
        if params.messages.is_empty() {
            return Err(invalid_request("messages must not be empty"));
        }
        if params.messages.len() > MAX_MESSAGES {
            return Err(invalid_request(format!(
                "messages must contain at most {MAX_MESSAGES} entries"
            )));
        }
        let mut seen = std::collections::HashSet::with_capacity(params.messages.len());
        let queries = params
            .messages
            .iter()
            .map(|message| {
                normalize_external_message_id(&message.message_id).map_err(invalid_request)?;
                if !seen.insert(message.message_id.as_str()) {
                    return Err(invalid_request(
                        "messages must contain unique messageId values",
                    ));
                }
                if message.semantic_sha256.len() != 64
                    || !message
                        .semantic_sha256
                        .bytes()
                ...
                {
                    return Err(invalid_request(
                        "semanticSha256 must be 64 lowercase hexadecimal characters",
                    ));
                }
                Ok(CoreInterAgentDeliveryQuery {
                    message_id: message.message_id.clone(),
                    semantic_sha256: message.semantic_sha256.clone(),
                })
            })
            .collect::<Result<Vec<_>, JSONRPCErrorError>>()?;
        let (_, thread) = self.load_thread(&params.thread_id).await?;
        let statuses = thread
            .inter_agent_delivery_status(&queries)
            .await
            .into_iter()
            .map(|status| match status.state {
                CoreInterAgentDeliveryState::Unknown => InterAgentDeliveryStatus::Unknown {
                    message_id: status.message_id,
                    semantic_sha256: status.semantic_sha256,
                },
                CoreInterAgentDeliveryState::Pending => InterAgentDeliveryStatus::Pending {
                    message_id: status.message_id,
                    semantic_sha256: status.semantic_sha256,
                },
                CoreInterAgentDeliveryState::ContextPersisted(receipt) => {
                    InterAgentDeliveryStatus::ContextPersisted {
                        message_id: status.message_id,
                        semantic_sha256: status.semantic_sha256,
                        receipt: InterAgentDeliveryReceipt {
                            receipt_id: receipt.receipt_id,
                            response_item_id: receipt.response_item_id,
                            turn_id: receipt.turn_id,
                            rollout_ordinal: receipt.rollout_ordinal,
                        },
                    }
                }
                CoreInterAgentDeliveryState::Conflict => InterAgentDeliveryStatus::Conflict {
                    message_id: status.message_id,
                    semantic_sha256: status.semantic_sha256,
                },
                CoreInterAgentDeliveryState::RetryableError => {
                    InterAgentDeliveryStatus::RetryableError {
                        message_id: status.message_id,
                        semantic_sha256: status.semantic_sha256,
                    }
                }
    ...
    async fn thread_cutex_activity_response_inner(
        &self,
        request_id: &ConnectionRequestId,
        params: ThreadCutexActivityParams,
    ) -> Result<ThreadCutexActivityResponse, JSONRPCErrorError> {
        params.validate().map_err(invalid_request)?;
        let (_, thread) = self.load_thread(&params.thread_id).await?;
        // Cutex timeline ingestion must not become a side door for writing to native
        // parent-owned child threads.
        self.ensure_direct_input_allowed(request_id, thread.as_ref())
            .await?;
        ...
        let submission_id = self
            ...
            .await
            .map_err(|err| internal_error(format!("failed to submit Cutex activity: {err}")))?;
        let disposition = reply_rx
            .await
            .map_err(|_| internal_error("Cutex activity handler dropped its reply".to_string()))?
            .map_err(|err| internal_error(format!("failed to record Cutex activity: {err}")))?;

        Ok(ThreadCutexActivityResponse {
            submission_id,
