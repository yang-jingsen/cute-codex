...
async fn turn_start_with_empty_input_runs_model_request() -> Result<()> {
    ...
    assert_eq!(
        ...
        "mock_provider",
        "ambient launch configuration must not override the hermetic mock provider"
    );
...
async fn direct_input_to_multi_agent_v2_subagent_is_rejected(
    ...
    let direct_cutex_activity_req = mcp
        .send_raw_request(
            "thread/cutexActivity",
            Some(json!({
                ...
                "activity": {
                    ...
                    "id": "fabricated-managed-action",
                    "eventId": "fabricated-managed-event",
                    "sequence": 1,
                    "occurredAtMs": 1,
                    "operation": "create",
                    "status": "completed",
                    "managedAgentId": "cutex.worker",
                    "managedAgentName": "Worker",
                    "managedAgentRole": "worker",
                    "initialTaskPreview": null,
                    "detail": null,
                    "runtimeGeneration": 1
                }
            })),
        ...
        .await?;
    let direct_cutex_activity_error: JSONRPCError = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_error_message(RequestId::Integer(direct_cutex_activity_req)),
    )
    .await??;
    assert_eq!(
        direct_cutex_activity_error.error.code,
        INVALID_REQUEST_ERROR_CODE
    );
    assert_eq!(direct_cutex_activity_error.error.message, ERROR_MESSAGE);
