...
async fn wait_for_process_marker(marker: &str, should_exist: bool) -> Result<()> {
    ...
    loop {
        let matching_processes = process_lines_with_marker(marker)?;
        if matching_processes.is_empty() != should_exist {
            return Ok(());
        }
        if Instant::now() >= deadline {
            let expectation = if should_exist { "appear" } else { "exit" };
            anyhow::bail!(
                "process marker {marker:?} did not {expectation} before timeout; matching: {matching_processes:#?}"
            );
        }
...
fn process_lines_with_marker(marker: &str) -> Result<Vec<String>> {
