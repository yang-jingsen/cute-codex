...
- `thread/inter_agent_message` — accept an identified inter-agent mailbox message for a loaded thread. Pass required `threadId`, non-empty `messageId`, absolute `author` and `recipient` AgentPath strings, `content`, and `deliveryMode` (`after_turn`, `soon`, `passive`, or `interrupt`); `otherRecipients` is optional. The response contains only `submissionId`, which confirms native submission acceptance rather than model consumption. When the message is actually consumed, it appears once as an `interAgentMessage` through the ordinary `item/started` and `item/completed` lifecycle and persisted thread history.
- `thread/inter_agent_message/status` — query up to 256 unique `(messageId, semanticSha256)` pairs in request order without serializing behind thread work. The response schema is `cutex/inter-agent-delivery-status/v1`; each entry is `unknown`, `pending`, `context_persisted`, `conflict`, or `retryable_error`. Only `context_persisted` includes a stable receipt proving that the canonical response item was appended and its rollout durably flushed. A successful `thread/inter_agent_message` submission and UI item lifecycle are not that proof.

  `semanticSha256` is SHA-256 over `cutex:inter-agent-message-semantic:v1\0` followed by the canonical tuple `messageId`, `author`, `recipient`, `otherRecipients`, `deliveryMode`, and `content`. Encode every string as an unsigned 64-bit big-endian byte length followed by its UTF-8 bytes; encode `otherRecipients` as an unsigned 64-bit big-endian count followed by each length-prefixed recipient in order. Delivery modes use their snake-case wire names. Presentation metadata is excluded.
