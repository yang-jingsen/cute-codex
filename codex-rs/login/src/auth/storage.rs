...
#[cfg(test)]
use super::resolve_auth_file_path;
use super::storage_identity::AuthStorageIdentity;
...
/// Expected structure for the selected auth file (default: `$CODEX_HOME/auth.json`).
...
pub(super) fn get_auth_file(codex_home: &Path) -> PathBuf {
    resolve_auth_file_path(codex_home)
}
...
impl FileAuthStorage {
    ...
    pub(super) fn new(codex_home: PathBuf) -> Self {
        Self::from_identity(&AuthStorageIdentity::from_env(codex_home))
    }

    fn from_identity(identity: &AuthStorageIdentity) -> Self {
        Self {
            auth_file: identity.auth_file.clone(),
        }
    }

    /// Attempt to read and parse the selected auth file.
...
#[cfg(test)]
static CODEX_AUTH_SECRET_NAME: Lazy<SecretName> = Lazy::new(|| {
    SecretName::new("CODEX_AUTH")
        .unwrap_or_else(|err| unreachable!("CODEX_AUTH should be valid: {err}"))
});

#[cfg(test)]
fn compute_store_key(codex_home: &Path) -> std::io::Result<String> {
    Ok(AuthStorageIdentity::from_value(codex_home.to_path_buf(), None).direct_store_key)
}
...
impl DirectKeyringAuthStorage {
    ...
    fn new(codex_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self::from_identity(AuthStorageIdentity::from_env(codex_home), keyring_store)
    }

    fn from_identity(identity: AuthStorageIdentity, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            identity,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        self.load_from_keyring(&self.identity.direct_store_key)
    }

    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        // Simpler error mapping per style: prefer method reference over closure
        let serialized = serde_json::to_string(auth).map_err(std::io::Error::other)?;
        self.save_to_keyring(&self.identity.direct_store_key, &serialized)?;
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .keyring_store
            .delete(KEYRING_SERVICE, &self.identity.direct_store_key)
            .map_err(|err| {
...
impl Debug for SecretsKeyringAuthStorage {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SecretsKeyringAuthStorage")
            .field("codex_home", &self.identity.codex_home)
...
impl SecretsKeyringAuthStorage {
    ...
    fn from_identity(identity: AuthStorageIdentity, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::from_identity(identity.clone(), Arc::clone(&keyring_store));
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &self.identity.encrypted_secret_name)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(
                &SecretScope::Global,
                &self.identity.encrypted_secret_name,
                &serialized,
            )
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &self.identity.encrypted_secret_name)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    ...
    fn new(
        ...
        Self::from_identity(
            AuthStorageIdentity::from_env(codex_home),
            keyring_store,
    ...
    fn from_identity(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::from_identity(&identity)),
        }
    }
}
...
impl EphemeralAuthStorage {
    ...
    {
        let key = self.identity.direct_store_key.clone();
...
fn create_auth_storage_with_store(
    ...
    let identity = AuthStorageIdentity::from_env(codex_home);
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::from_identity(&identity)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage_with_identity(identity, keyring_store, keyring_backend_kind)
        }
        AuthCredentialsStoreMode::Auto => Arc::new(AutoAuthStorage::from_identity(
            identity,
            keyring_store,
            keyring_backend_kind,
        )),
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage { identity }),
    }
}
...
fn create_keyring_auth_storage_with_identity(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => Arc::new(DirectKeyringAuthStorage::from_identity(
            identity,
            keyring_store,
        )),
        AuthKeyringBackendKind::Secrets => Arc::new(SecretsKeyringAuthStorage::from_identity(
            identity,
            keyring_store,
        )),
    }
