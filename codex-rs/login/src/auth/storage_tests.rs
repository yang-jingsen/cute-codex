...
#[test]
fn auth_file_overrides_isolate_all_storage_backends() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let default_identity = AuthStorageIdentity::from_value(codex_home.path().to_path_buf(), None);
    let first_identity = AuthStorageIdentity::from_value(
        codex_home.path().to_path_buf(),
        Some("profiles/first-auth.json"),
    );
    let second_identity = AuthStorageIdentity::from_value(
        codex_home.path().to_path_buf(),
        Some("profiles/second-auth.json"),
    );
    let first_auth = auth_with_prefix("first-profile");
    let second_auth = auth_with_prefix("second-profile");

    let first_file = FileAuthStorage::from_identity(&first_identity);
    let second_file = FileAuthStorage::from_identity(&second_identity);
    first_file.save(&first_auth)?;
    second_file.save(&second_auth)?;
    assert_eq!(first_file.load()?, Some(first_auth.clone()));
    assert_eq!(second_file.load()?, Some(second_auth.clone()));
    assert!(!default_identity.auth_file.exists());

    let direct_keyring = Arc::new(MockKeyringStore::default());
    let first_direct =
        DirectKeyringAuthStorage::from_identity(first_identity.clone(), direct_keyring.clone());
    let second_direct =
        DirectKeyringAuthStorage::from_identity(second_identity.clone(), direct_keyring.clone());
    let default_direct =
        DirectKeyringAuthStorage::from_identity(default_identity.clone(), direct_keyring);
    first_direct.save(&first_auth)?;
    second_direct.save(&second_auth)?;
    assert_eq!(first_direct.load()?, Some(first_auth.clone()));
    assert_eq!(second_direct.load()?, Some(second_auth.clone()));
    assert_eq!(default_direct.load()?, None);

    let secrets_keyring = Arc::new(MockKeyringStore::default());
    let first_secrets =
        SecretsKeyringAuthStorage::from_identity(first_identity.clone(), secrets_keyring.clone());
    let second_secrets =
        SecretsKeyringAuthStorage::from_identity(second_identity.clone(), secrets_keyring.clone());
    let default_secrets =
        SecretsKeyringAuthStorage::from_identity(default_identity.clone(), secrets_keyring);
    first_secrets.save(&first_auth)?;
    second_secrets.save(&second_auth)?;
    assert_eq!(first_secrets.load()?, Some(first_auth.clone()));
    assert_eq!(second_secrets.load()?, Some(second_auth.clone()));
    assert_eq!(default_secrets.load()?, None);

    let first_ephemeral = EphemeralAuthStorage {
        identity: first_identity,
    };
    let second_ephemeral = EphemeralAuthStorage {
        identity: second_identity,
    };
    let default_ephemeral = EphemeralAuthStorage {
        identity: default_identity,
    };
    first_ephemeral.save(&first_auth)?;
    second_ephemeral.save(&second_auth)?;
    assert_eq!(first_ephemeral.load()?, Some(first_auth));
    assert_eq!(second_ephemeral.load()?, Some(second_auth));
    assert_eq!(default_ephemeral.load()?, None);
