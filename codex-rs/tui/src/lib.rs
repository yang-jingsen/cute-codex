...
mod cutex_activities;
mod cutex_management_phases;
mod cutex_rich_text;
mod cutex_runtime_heartbeat;
mod cutex_shared_config;
mod cutex_template_groups;
mod pets;
mod task_service_messages;
...
fn can_reuse_implicit_local_daemon(
    ...
    loader_overrides: &LoaderOverrides,
    strict_config: bool,
    has_profile_file_overrides: bool,
...
async fn run_ratatui_app(
    ...
    let mut config = match reloaded_config {
        Ok(mut config) => {
            cutex_shared_config::apply_shared_cutex_tui_settings(&mut config);
            config
        }
        Err(err) => {
            shutdown_startup_session(app_server.take(), &mut terminal_restore_guard).await;
...
mod tests {
    ...
    fn can_reuse_implicit_local_daemon_requires_default_launch_config() -> color_eyre::Result<()> {
        ...
        assert!(can_reuse_implicit_local_daemon(
            ...
            &LoaderOverrides::default(),
            /*strict_config*/ false,
            /*has_profile_file_overrides*/ false,
        ...
        assert!(!can_reuse_implicit_local_daemon(
            ...
            &LoaderOverrides::default(),
            /*strict_config*/ false,
            /*has_profile_file_overrides*/ true,
            /*has_non_replayable_launch_overrides*/ false,
        ));
        Ok(())
    }
