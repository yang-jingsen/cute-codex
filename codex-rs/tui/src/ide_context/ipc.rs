...
mod tests {
    ...
    fn private_testdir() -> std::io::Result<tempfile::TempDir> {
    ...
    #[test]
    fn fetch_ide_context_prefers_primary_socket() -> Result<(), Box<dyn std::error::Error>> {
        use std::os::unix::net::UnixListener;

        let tempdir = private_testdir()?;
    ...
    #[test]
    fn fetch_ide_context_falls_back_to_legacy_socket() -> Result<(), Box<dyn std::error::Error>> {
        use std::os::unix::net::UnixListener;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        std::fs::set_permissions(legacy_parent, std::fs::Permissions::from_mode(0o700))
            .expect("make legacy socket directory private");
        let legacy_listener = UnixListener::bind(&legacy_socket_path).expect("bind legacy");
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let pre_migration_parent = pre_migration_socket_path.parent().expect("legacy parent");
        std::fs::create_dir(pre_migration_parent).expect("create legacy parent");
        std::fs::set_permissions(pre_migration_parent, std::fs::Permissions::from_mode(0o700))
            .expect("make legacy socket directory private");
        let legacy_listener =
    ...
    #[cfg(unix)]
    #[test]
    fn fetch_ide_context_uses_unregistered_request_route() -> Result<(), Box<dyn std::error::Error>>
    {
        use std::os::unix::net::UnixListener;
        use std::thread;
