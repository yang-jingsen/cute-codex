    ...
    loop {
        ...
        match line.item {
            ...
            RolloutItem::EventMsg(EventMsg::ItemCompleted(event)) => {
                if let codex_protocol::items::TurnItem::InterAgentMessage(message) = event.item {
                    let author = message.author.to_string();
                    let recipient = message.recipient.to_string();
                    let other_recipients = message
                        .other_recipients
                        .iter()
                    ...
                    append_transcript_preview_text(
                        &mut lines,
                        TranscriptPreviewSpeaker::InterAgent,
...
fn append_transcript_preview_text(
    ...
    let text = match speaker {
        ...
        TranscriptPreviewSpeaker::InterAgent => Cow::Borrowed(text),
        TranscriptPreviewSpeaker::Assistant => {
