    ...
    let mut items: Vec<Value> = turn
        ...
        .map(|item| match item {
            ...
            ThreadItem::InterAgentMessage {
                ...
                "type": "interAgentMessage", "id": id, "author": author,
                "recipient": recipient, "otherRecipients": other_recipients,
                "content": truncate(content, DEFAULT_OUTPUT_CHARS),
            ...
            ThreadItem::ManagedAgentActivity { activity } => json!({
                "type": "managedAgentActivity", "activity": activity
            }),
            ThreadItem::OutboundInterAgentMessage { activity } => json!({
                "type": "outboundInterAgentMessage", "activity": activity
            }),
            ThreadItem::TaskAssignmentActivity { activity } => json!({
                "type": "taskAssignmentActivity", "activity": activity
            }),
            ThreadItem::TaskWatchdogActivity { activity } => json!({
                "type": "taskWatchdogActivity", "activity": activity
            }),
