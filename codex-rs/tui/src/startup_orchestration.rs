...
pub(super) async fn run_main_inner(
    ...
    let has_profile_file_overrides = codex_config::user_config_path_from_env(&codex_home).is_some()
        || codex_login::auth_file_path_from_env(&codex_home).is_some();
    let has_non_replayable_launch_overrides =
        cli.bypass_hook_trust || codex_app_server_client::force_http_transport_enabled();
