...
#[cfg(test)]
use crate::inter_agent_messages::InterAgentMessagePresentation;
...
fn render_transcript_content_lines(line: &TranscriptPreviewLine, width: u16) -> Vec<Line<'static>> {
    ...
    let lines = match line.speaker {
        ...
        TranscriptPreviewSpeaker::InterAgent => vec![conversation_content_line(
            Line::from(line.text.clone()),
...
"#
    ...
    #[test]
    fn inter_agent_conversation_preview_snapshot() {
        let message = InterAgentMessagePresentation::new(
            "message-1",
            "/root/coordinator",
            "/root/worker",
            &[],
            "Continue with the focused TUI tests.",
            codex_app_server_protocol::InterAgentDeliveryMode::AfterTurn,
        );
        let lines = render_conversation_preview_lines(
            &[TranscriptPreviewLine {
                speaker: TranscriptPreviewSpeaker::InterAgent,
                text: message.compact_summary(),
            }],
            /*width*/ 80,
        );
        let rendered = lines
        ...
        assert_snapshot!("resume_picker_inter_agent_conversation_preview", rendered);
    }
