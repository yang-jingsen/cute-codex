...
fn final_message_separator_hides_short_worked_label_and_includes_runtime_metrics() {
    ...
    let cell = FinalMessageSeparator::new(0, Some(12), Some(summary));
    let rendered = render_lines(&cell.display_lines(/*width*/ 600));

    assert_eq!(rendered.len(), 1);
    assert!(rendered[0].contains(&format_local_compact_timestamp(0)));
...
fn final_message_separator_includes_worked_label_after_one_minute() {
    let cell = FinalMessageSeparator::new(0, Some(61), /*runtime_metrics*/ None);
...
#[test]
fn final_message_separator_pure_and_worked_variants_always_show_occurrence_time() {
    let timestamp = 1_725_000_123;
    let expected = format_local_compact_timestamp(timestamp);
    let pure = FinalMessageSeparator::new(timestamp, /*elapsed_seconds*/ None, None);
    let short = FinalMessageSeparator::new(timestamp, Some(60), None);
    let long = FinalMessageSeparator::new(timestamp, Some(61), None);

    let pure = render_lines(&pure.display_lines(/*width*/ 80)).join("\n");
    let short = render_lines(&short.display_lines(/*width*/ 80)).join("\n");
    let long = render_lines(&long.display_lines(/*width*/ 80)).join("\n");
    assert!(pure.contains(&expected));
    assert!(short.contains(&expected));
    assert!(long.contains(&expected));
    assert!(!pure.contains("Worked for"));
    assert!(!short.contains("Worked for"));
    assert!(long.contains("Worked for 1m 01s"));
}

#[test]
fn compact_timestamp_formatting_is_timezone_independent() {
    let east = FixedOffset::east_opt(10 * 60 * 60).expect("valid offset");
    let west = FixedOffset::west_opt(8 * 60 * 60).expect("valid offset");
    assert_eq!(format_compact_timestamp(0, &east).as_deref(), Some("10:00"));
    assert_eq!(format_compact_timestamp(0, &west).as_deref(), Some("16:00"));
}
