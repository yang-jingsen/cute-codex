...
pub(crate) const APPROVAL_PROMPT_TYPING_IDLE_DELAY: Duration = Duration::from_secs(1);
...
mod tests {
    ...
    #[test]
    fn terminal_sideband_modes_track_composer_task_and_views() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventSender::new(tx_raw);
        let mut pane = test_pane(tx);
        let area = Rect::new(0, 0, 80, 12);

        let state = pane.terminal_sideband_state(area, 80, 24);
        assert_eq!(state.mode, "editing");
        assert!(state.input_ready);
        assert!(state.composer.visible);

        pane.set_task_running(/*running*/ true);
        let state = pane.terminal_sideband_state(area, 80, 24);
        assert_eq!(state.mode, "assistant_running");
        assert!(!state.input_ready);
        assert!(state.composer.visible);
        assert!(!state.composer.focused);
        ...
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventSender::new(tx_raw);
        let mut approval_pane = test_pane(tx);
        approval_pane.push_approval_request(exec_request(), &Features::with_defaults());
        let state = approval_pane.terminal_sideband_state(area, 80, 24);
        assert_eq!(state.mode, "approval_prompt");
        assert!(state.input_ready);
        assert!(!state.composer.visible);
    }
