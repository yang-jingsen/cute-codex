...
//! `BottomPane` also records each user-visible composer mutation with a monotonic
//! activity generation so lifecycle consumers can distinguish an unchanged draft
//! from one that was edited and then cleared.
//!
...
mod tests {
    ...
    fn terminal_sideband_reports_utf16_cursor_and_absolute_caret() {
        ...
        composer.draft.textarea.set_cursor("a😀".len());
