...
impl ChatWidget {
    pub(crate) fn handle_server_notification(
        ...
        let previous_notify_service_replay_suppressed = self.notify_service_replay_suppressed;
        self.notify_service_replay_suppressed =
            previous_notify_service_replay_suppressed || from_replay;
        ...
        match notification {
            ...
            ServerNotification::TurnStarted(notification) => {
                ...
                if !matches!(replay_kind, Some(ReplayKind::ResumeInitialMessages)) {
                    if replay_kind.is_some() {
                        self.on_task_started_from_replay();
                    } else {
            ...
            ServerNotification::CutexActivityPresented(notification) => {
                if matches!(
                    notification.delivery.class,
                    CutexUiActivityDeliveryClass::Live
                ) && !notification.delivery.recovered
                {
                    self.on_cutex_activity(notification.item);
                } else {
                    self.on_recovered_cutex_activity(notification.delivery, notification.item);
                }
            }
        ...
        self.notify_service_replay_suppressed = previous_notify_service_replay_suppressed;
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                    ...
                    if replay_kind.is_none() {
                        self.emit_turn_failed_notification("turn failed without an error message");
                    }
                    self.finalize_turn();
                    self.request_redraw();
    ...
    fn handle_item_started_notification(
        ...
        if !from_replay {
            ...
            if owns_final_message_divider(&notification.item) {
                self.transcript.last_live_activity_occurred_at_seconds =
                    Some(notification.started_at_ms / 1_000);
            }
        }
        match notification.item {
            ...
            item @ (ThreadItem::ManagedAgentActivity { .. }
            | ThreadItem::OutboundInterAgentMessage { .. }
            | ThreadItem::TaskAssignmentActivity { .. }
            | ThreadItem::TaskWatchdogActivity { .. }) => self.on_cutex_activity(item),
    ...
    fn handle_item_completed_notification(
        ...
        if replay_kind.is_none() {
            ...
            if owns_final_message_divider(&notification.item) {
                self.transcript.last_live_activity_occurred_at_seconds =
                    Some(notification.completed_at_ms / 1_000);
            }
        }
        match notification.item {
...
fn owns_final_message_divider(item: &ThreadItem) -> bool {
    matches!(
        item,
