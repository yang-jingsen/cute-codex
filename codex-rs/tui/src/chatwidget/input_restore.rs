...
impl ChatWidget {
    ...
    pub(super) fn pop_next_queued_user_message(
        ...
        if self.input_queue.rejected_steers_queue.is_empty() {
            ...
            // A steer reaches this queue only after its app-server submission
            // was accepted. Preserve the sent-notification disposition when
            // retrying it as a fresh turn.
            Some((
                QueuedUserMessage::new_with_notify_sent(message, QueuedInputAction::Plain, true),
                history_record,
            ))
        }
    }
