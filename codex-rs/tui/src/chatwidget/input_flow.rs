...
impl ChatWidget {
    ...
    pub(super) fn queue_user_message_with_options(
        ...
        if !should_run_now || action != QueuedInputAction::Plain {
            ...
            let notify_sent = self
                .emit_user_message_sent_notification_with_history(&user_message, &history_record);
            self.input_queue.queued_user_messages.push_back(
                QueuedUserMessage::new_with_pending_pastes_and_notify_sent(
                    user_message,
                    action,
                    pending_pastes,
                    notify_sent,
    ...
    pub(crate) fn maybe_send_next_queued_input(&mut self) -> bool {
        ...
        while !self.is_user_turn_pending_or_running() {
            ...
            let (user_message, action, pending_pastes, notify_sent) =
                queued_message.into_parts_with_pending_pastes();
