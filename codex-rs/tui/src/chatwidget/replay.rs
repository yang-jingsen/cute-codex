    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
            ...
            ThreadItem::InterAgentMessage {
                ...
                let cutex_settings = self.config.tui_cutex_activity.inbound_message.clone();
                let task_service_settings =
                    self.config.tui_cutex_activity.task_service_message.clone();
                let task_service_template = (!from_replay)
                    .then(|| {
                        task_service_presentation.as_ref().map(|presentation| {
                            self.cutex_template_correlations
                                .task_service_decision(presentation, &task_service_settings)
                        })
                    })
                    .flatten()
                    .flatten();
                ...
                .with_cutex_metadata(
                    author_metadata.as_ref(),
                    recipient_metadata.as_ref(),
                    Some(&cutex_settings),
                )
                .with_task_service_presentation(
                    task_service_presentation.as_ref(),
                    Some(&task_service_settings),
                )
                .with_task_service_template(task_service_template.as_ref());
                let history_cell = message.history_cell();
