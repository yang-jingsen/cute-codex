...
impl ChatWidget {
    ...
    pub(super) fn on_task_started(&mut self) {
        self.on_task_started_with_notify(/*from_replay*/ false);
    }

    pub(super) fn on_task_started_from_replay(&mut self) {
        self.on_task_started_with_notify(/*from_replay*/ true);
    }

    fn on_task_started_with_notify(&mut self, from_replay: bool) {
        if !from_replay {
    ...
    pub(super) fn on_task_complete(
        ...
        if !from_replay {
            ...
            if show_work_separator || runtime_metrics.is_some() {
                ...
                self.add_to_history(history_cell::FinalMessageSeparator::new(
                    completed_at.unwrap_or_else(history_cell::current_unix_timestamp_seconds),
                    elapsed_seconds,
                    runtime_metrics,
                ));
            }
        ...
        // `turn_completed` is an immediate lifecycle receipt. The delayed
        // `task_completed` idle event below is reserved for the true
        // user-attention boundary, so retain both follow-up facts here.
        if !from_replay {
            self.post_notify_service_event(
    ...
    pub(super) fn emit_turn_failed_notification(&self, message: &str) {
        self.post_notify_service_event(
    ...
    pub(super) fn emit_turn_interrupted_notification(&self, reason: TurnAbortReason) {
        self.post_notify_service_event(
