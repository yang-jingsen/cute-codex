...
impl ChatWidget {
    ...
    pub(crate) fn setup_status_line(&mut self, ids: Vec<String>, use_theme_colors: bool) {
        tracing::info!(
            "status line setup confirmed with ids: {ids:#?}, use_theme_colors: {use_theme_colors}"
        );
    ...
    pub(super) fn open_status_line_setup(&mut self) {
        ...
        let available_items = self.available_status_line_choices(&preview_data);
