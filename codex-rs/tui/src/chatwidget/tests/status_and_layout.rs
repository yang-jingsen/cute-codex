...
#[tokio::test]
async fn status_line_builtin_aliases_deduplicate_to_one_canonical_item() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.tui_status_line = Some(vec![
    ...
    let rendered = status_line_text(&chat).expect("model status should render");
    assert!(
        !rendered.contains(" · "),
        "alias rendered more than once: {rendered}"
    );
}
...
async fn completed_turn_refreshes_estimated_thread_cost() {
    ...
    chat.on_task_complete(
        /*last_agent_message*/ None, /*duration_ms*/ None, /*completed_at*/ None,
        /*from_replay*/ false,
    );
...
struct StatusLineEnvVarGuard {
    key: &'static str,
    original: Option<std::ffi::OsString>,
}

impl StatusLineEnvVarGuard {
    fn set(key: &'static str, value: impl AsRef<std::ffi::OsStr>) -> Self {
        let original = std::env::var_os(key);
        // SAFETY: these tests share one serial-test key and restore the variable on drop.
...
impl Drop for StatusLineEnvVarGuard {
    fn drop(&mut self) {
        // SAFETY: these tests share one serial-test key and restore the variable on drop.
        unsafe {
...
async fn status_line_launch_profile_and_runtime_render_when_set() {
    let _catalog = StatusLineEnvVarGuard::remove(
        crate::custom_status_items::CODEX_CUSTOM_STATUS_ITEMS_FILE_ENV_VAR,
    );
    let _profile = StatusLineEnvVarGuard::set(
        crate::custom_status_items::CODEX_LAUNCH_PROFILE_ENV_VAR,
        "  sample-profile  ",
    );
    let _runtime = StatusLineEnvVarGuard::set(
        crate::custom_status_items::CODEX_LAUNCH_RUNTIME_ENV_VAR,
        "docker",
    );
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.tui_status_line = Some(vec![
    ...
    // SAFETY: this test is serialized with every mutation of these variables,
    // and the guards restore their original values when the test exits.
    unsafe {
        std::env::set_var(
            crate::custom_status_items::CODEX_LAUNCH_PROFILE_ENV_VAR,
            "   ",
        );
        std::env::remove_var(crate::custom_status_items::CODEX_LAUNCH_RUNTIME_ENV_VAR);
    }
    chat.refresh_status_line();
    assert_eq!(status_line_text(&chat), None);
}
...
async fn status_line_custom_catalog_item_renders_when_selected() {
    ...
    std::fs::write(
        ...
        r##"{
            "items": [{
                ...
                "render": { "kind": "label_value", "label": "Catalog" },
                "style": { "fg": "#F6A3C8", "bold": true }
            }]
        }"##,
    )
    .expect("catalog should write");
    let _catalog = StatusLineEnvVarGuard::set(
        crate::custom_status_items::CODEX_CUSTOM_STATUS_ITEMS_FILE_ENV_VAR,
        &catalog_path,
    );
    let _profile = StatusLineEnvVarGuard::set(
    ...
    assert_eq!(
        status_line_text(&chat),
        Some("Profile sample-profile · Catalog sample-profile".to_string())
    );

    chat.open_status_line_setup();
    let popup = strip_osc8_for_snapshot(&render_bottom_popup(&chat, /*width*/ 100));
    assert!(
        popup.contains("[x] Profile"),
        "catalog choice missing: {popup}"
    );
    assert!(
        popup.contains("Launcher profile label"),
        "catalog description missing: {popup}"
    );
    assert!(
        popup.contains("Catalog sample-profile"),
        "catalog preview missing: {popup}"
    );
}

#[tokio::test]
#[serial(custom_status_environment)]
async fn status_line_builtin_ids_cannot_be_shadowed_by_catalog() {
    let tempdir = tempdir().expect("tempdir");
    let catalog_path = tempdir.path().join("custom-status-items.json");
    std::fs::write(
        &catalog_path,
        r#"{
            "items": [{
                "id": "launch-profile",
                "title": "Spoofed profile",
                "source": { "kind": "static", "value": "spoofed" }
            }]
        }"#,
    )
    .expect("catalog should write");
    ...
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.tui_status_line = Some(vec!["launch-profile".to_string()]);

    chat.refresh_status_line();
    let preview_data = chat.status_surface_preview_data();
    let choices = chat.available_status_line_choices(&preview_data);

    assert_eq!(
        status_line_text(&chat),
        Some("Profile real-profile".to_string())
    );
    assert_eq!(
        choices
            .iter()
            .filter(|choice| choice.id == "launch-profile")
            .count(),
        1
    );
}
