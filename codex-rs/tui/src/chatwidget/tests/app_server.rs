...
#[tokio::test]
async fn live_cutex_outbound_activity_updates_one_history_cell() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let activity = codex_app_server_protocol::OutboundInterAgentMessageItem {
        id: "outbound-1".to_string(),
        event_id: "event-1".to_string(),
        sequence: 1,
        occurred_at_ms: 1_725_000_123_000,
        sender_agent_id: "director".to_string(),
        sender_agent_name: Some("Director".to_string()),
        sender_metadata: None,
        recipient_agent_id: "worker".to_string(),
        recipient_agent_name: Some("Worker".to_string()),
        ...
        status: codex_app_server_protocol::OutboundInterAgentMessageStatus::Sending,
        content_preview: Some("implement the focused change".to_string()),
        detail: None,
    };
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            started_at_ms: activity.occurred_at_ms,
            item: AppServerThreadItem::OutboundInterAgentMessage {
    ...
    chat.handle_server_notification(
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: activity.occurred_at_ms + 10,
            item: AppServerThreadItem::OutboundInterAgentMessage {
                activity: codex_app_server_protocol::OutboundInterAgentMessageItem {
                    event_id: "event-2".to_string(),
                    sequence: 2,
                    status: codex_app_server_protocol::OutboundInterAgentMessageStatus::Sent,
                    ..activity
                },
            },
        }),
    ...
    chat.handle_streaming_delta("Final response".to_string());
    let divider = drain_insert_history(&mut rx)
        .into_iter()
        .map(|lines| lines_to_single_string(&lines))
        .find(|line| line.contains('─'))
        .expect("live activity should create a final-message divider");
    assert!(
        divider.contains(&crate::history_cell::format_local_compact_timestamp(
            1_725_000_123
        ))
    );
}
