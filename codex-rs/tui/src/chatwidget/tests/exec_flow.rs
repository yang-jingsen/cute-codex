...
async fn final_worked_for_uses_cumulative_turn_duration_snapshot() {
    for duration_ms in [Some(125_000), None] {
        ...
        assert!(
            combined.contains(&crate::history_cell::format_local_compact_timestamp(
                1_725_000_123
            )),
            "expected final separator to use turn completion time, got:\n{combined}"
        );
        let snapshot = combined
            .replace(
                &crate::history_cell::format_local_compact_timestamp(0),
                "<time>",
            )
            .replace(
                &crate::history_cell::format_local_compact_timestamp(1_725_000_123),
                "<time>",
            );
