//! Structured HTTP notification hooks and idle state for `ChatWidget`.
...
impl ChatWidget {
    fn notify_service_enabled(&self) -> bool {
        !self.notify_service_replay_suppressed
            && self
                .config
                .notify_service
                .notify_service_url
    ...
    pub(super) fn post_notify_service_event(
        ...
        let settings = &self.config.notify_service;
        ...
        if !crate::notify_service::event_enabled(settings, status) {
            return false;
        }
        let cwd = self
            .current_cwd
        ...
        let payload = crate::notify_service::build_payload_with_details(
            status,
            &cwd,
            &settings.notify_service_agent_name,
        ...
        crate::notify_service::send_idle_notification(
            url.clone(),
            settings.notify_service_token.clone(),
            payload,
        );
        true
    }
            ...
            .notify_service_user_message_content
        {
            ...
            NotifyServiceUserMessageContent::Preview => {
                ...
                user_message_details.insert(
                    ...
                    serde_json::Value::Bool(text_chars > max_chars),
                );
            }
    ...
    pub(super) fn emit_user_message_sent_notification_with_history(
        &mut self,
        user_message: &UserMessage,
        _history_record: &UserMessageHistoryRecord,
    ) -> bool {
        self.clear_idle_state();
        self.clear_session_startup_idle_state();
        self.post_notify_service_event(
            NotifyServiceEvent::UserMessageSent,
            None,
            0,
            Some(self.user_message_notify_details(user_message)),
        )
    }
    ...
    pub(super) fn emit_user_message_sent_notification(
        ...
        self.emit_user_message_sent_notification_with_history(
            user_message,
            &UserMessageHistoryRecord::UserMessageText,
        )
    }

    pub(super) fn emit_user_message_dispatched_notification_with_history(
        &self,
        user_message: &UserMessage,
        _history_record: &UserMessageHistoryRecord,
    ...
    pub(super) fn maybe_emit_session_started_notification(
        ...
        if display != SessionConfiguredDisplay::Normal
            || previous_thread_id == Some(session.thread_id)
        ...
        let details = serde_json::json!({
            "session": {
                ...
                "forked_from_id": session.forked_from_id.map(|id| id.to_string())
            }
        });
    ...
    pub(crate) fn emit_approval_resolved_notification(
        ...
        let details = match request {
            ResolvedAppServerRequest::ExecApproval { id, .. } => {
                serde_json::json!({"approval": {"kind": "exec", "approval_id": id, "item_id": id}})
            }
            ResolvedAppServerRequest::FileChangeApproval { id, .. } => {
                serde_json::json!({"approval": {"kind": "patch", "item_id": id}})
            }
            ResolvedAppServerRequest::PermissionsApproval { id, .. } => {
                serde_json::json!({"approval": {"kind": "permissions", "item_id": id}})
            }
            ResolvedAppServerRequest::UserInput { call_id } => {
                serde_json::json!({"approval": {"kind": "user_input", "item_id": call_id}})
            }
        ...
        self.post_notify_service_event(NotifyServiceEvent::ApprovalResolved, None, 0, Some(details))
    }
        ...
        match item {
            ...
            ThreadItem::WebSearch(item) => {
                self.post_notify_service_event(
                    ...
                    self.current_turn_duration_seconds(),
                    0,
                    Some(serde_json::json!({ "web_search": { "item_id": item.id } })),
                );
            }
            ThreadItem::ImageGeneration(item) => {
                self.post_notify_service_event(
                    ...
                    0,
                    Some(serde_json::json!({
                        "image_generation": { "item_id": item.id }
                    })),
                );
            }
    ...
    pub(super) fn emit_completed_notification_for_thread_item(&self, item: &ThreadItem) {
        match item {
            ...
            ThreadItem::WebSearch(item) => {
                let action_type = match item.action.as_ref() {
                ...
                self.post_notify_service_event(
                    ...
                    0,
                    Some(serde_json::json!({
                        "web_search": { "item_id": item.id, "action_type": action_type }
                    })),
                );
            }
            ThreadItem::ImageGeneration(item) => {
    ...
    pub(super) fn idle_notification_due_at(
        ...
        if !matches!(
            ...
            return (elapsed >= timeout).then_some((status, elapsed.as_secs()));
        }
        if !self.idle_task_completed_composer_changed() {
            ...
            return (elapsed >= timeout
                && self.notify_service_event_enabled(NotifyServiceEvent::TaskCompleted))
            .then_some((status, elapsed.as_secs()));
        }
    ...
    pub(super) fn schedule_next_idle_notification_timer(&mut self) {
        let now = Instant::now();
        ...
        tokio::spawn(async move {
            ...
            tx.send(AppEvent::IdleNotifyTimerFired { generation });
        });
    }

    pub(super) fn schedule_idle_notification_after_composer_activity(
        &mut self,
        previous_generation: u64,
    ) {
        if previous_generation != self.bottom_pane.composer_activity_generation() {
            self.schedule_next_idle_notification_timer();
        }
    }
    ...
    pub(crate) fn handle_idle_notify_timer_fired(&mut self, generation: u64) {
        ...
        if let Some((status, idle_seconds)) = self.idle_notification_due_at(Instant::now()) {
    ...
    fn session_startup_idle_next_due_at(&self, now: Instant) -> Option<Instant> {
        ...
        self.session_startup_idle_entered_at?
            .checked_add(Duration::from_secs(
                self.config
                    .notify_service
                    .notify_service_startup_idle_timeout_secs,
    ...
    pub(super) fn schedule_next_session_startup_idle_timer(&mut self) {
        let now = Instant::now();
        ...
        tokio::spawn(async move {
            ...
            tx.send(AppEvent::SessionStartupIdleTimerFired { generation });
        });
    }
