...
impl ChatWidget {
    ...
    pub(super) fn on_cutex_activity(&mut self, item: ThreadItem) {
        if !cutex_activities::is_visible(&item, &self.config.tui_cutex_activity) {
            return;
        }
        self.transcript.last_live_activity_occurred_at_seconds =
            cutex_activities::occurred_at_ms(&item).map(|timestamp| timestamp / 1_000);
        self.transcript.had_work_activity = true;
        let Some(key) = cutex_activities::activity_key(&item, &self.config.tui_cutex_activity)
        else {
            return;
        };
        let terminal = cutex_activities::is_terminal(&item, &self.config.tui_cutex_activity);
        let template = match &item {
            ThreadItem::ManagedAgentActivity { activity } if activity.phase.is_some() => self
                .cutex_template_correlations
                .management_decision(activity, &self.config.tui_cutex_activity),
            ThreadItem::TaskWatchdogActivity { activity } => self
                .cutex_template_correlations
                .watchdog_decision(activity, &self.config.tui_cutex_activity),
            _ => None,
        };
        if let Some(handle) = self.cutex_activity_handles.get(&key) {
            handle.update(item, template);
            if terminal {
                self.cutex_activity_handles.remove(&key);
            }
            self.request_redraw();
            return;
        }
        if !cutex_activities::has_visible_content(&item, &self.config.tui_cutex_activity) {
            return;
        }

        let (cell, handle) = cutex_activities::new_activity_cell(
            item,
            template,
            self.config.tui_cutex_activity.clone(),
        );
        if !terminal {
            self.cutex_activity_handles.insert(key, handle);
        }
        self.flush_answer_stream_with_separator();
        self.add_to_history(cell);
        self.request_redraw();
    ...
    pub(super) fn on_recovered_cutex_activity(
        &mut self,
        delivery: CutexUiActivityDelivery,
        item: ThreadItem,
    ) {
        if !cutex_activities::is_visible(&item, &self.config.tui_cutex_activity)
            || !cutex_activities::has_visible_content(&item, &self.config.tui_cutex_activity)
        {
            return;
        }
        let Some(key) = cutex_activities::activity_key(&item, &self.config.tui_cutex_activity)
        ...
        let batch_id = delivery.batch_id.clone();
        let batch_complete = delivery.batch_index + 1 == delivery.batch_size;
        if let Some(handle) = self.recovered_cutex_activity_handles.get(&batch_id) {
            handle.update(key, delivery, item, template);
            if batch_complete {
                self.recovered_cutex_activity_handles.remove(&batch_id);
            }
            self.request_redraw();
            return;
        }

        let (cell, handle) = cutex_activities::new_recovered_activity_cell(
            key,
            delivery,
            item,
            template,
        ...
        if !batch_complete {
            self.recovered_cutex_activity_handles
                .insert(batch_id, handle);
        }
        self.flush_answer_stream_with_separator();
        self.add_to_history(cell);
        self.request_redraw();
