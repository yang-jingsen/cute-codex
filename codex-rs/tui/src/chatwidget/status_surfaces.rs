...
use crate::custom_status_items::CODEX_LAUNCH_PROFILE_ENV_VAR;
use crate::custom_status_items::CODEX_LAUNCH_RUNTIME_ENV_VAR;
...
impl StatusSurfaceSelections {
    fn uses_git_branch(&self) -> bool {
        status_line_selections_contain_builtin(&self.status_line_items, StatusLineItem::GitBranch)
            || self
    ...
    fn uses_git_summary(&self) -> bool {
        status_line_selections_contain_builtin(
...
fn status_line_selections_contain_builtin(
    selections: &[StatusLineSelection],
    item: StatusLineItem,
) -> bool {
    selections
        .iter()
        .any(|selection| matches!(selection, StatusLineSelection::Builtin(candidate) if *candidate == item))
}
...
impl ChatWidget {
    ...
    /// Parses configured status-line ids into native or catalog selections.
    ///
    /// Canonical aliases, catalog ids, and unknown ids are deduplicated in
    /// insertion order; unknown ids remain available for one warning message.
    fn status_line_items_with_invalids(&self) -> (Vec<StatusLineSelection>, Vec<String>) {
        ...
        for id in self.configured_status_line_items() {
            if let Ok(item) = id.parse::<StatusLineItem>() {
                ...
                .custom_status_items
                .iter()
                .any(|custom| custom.id == id)
            {
                if seen.insert(id.clone()) {
                    items.push(StatusLineSelection::Custom(id));
                }
    ...
    pub(super) fn refresh_status_line_if_workspace_headline_due(&mut self) {
        ...
        if self.status_line_workspace_headline_should_fetch(now)
            && status_line_selections_contain_builtin(
                &self.status_line_items_with_invalids().0,
                StatusLineItem::WorkspaceHeadline,
            )
        {
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::LaunchProfile => {
                launch_env_status_line_value(CODEX_LAUNCH_PROFILE_ENV_VAR, "Profile")
            }
            StatusLineItem::LaunchRuntime => {
                launch_env_status_line_value(CODEX_LAUNCH_RUNTIME_ENV_VAR, "Runtime")
            }
    ...
    pub(super) fn available_status_line_choices(
        &self,
        preview_data: &StatusSurfacePreviewData,
    ) -> Vec<StatusLineChoice> {
        use strum::IntoEnumIterator;

        let mut choices = StatusLineItem::iter()
            .map(|item| StatusLineChoice::builtin(item, preview_data))
            .collect::<Vec<_>>();
        let cwd = self.status_line_cwd();
        for custom in &self.custom_status_items {
            if custom.id.parse::<StatusLineItem>().is_ok() {
                continue;
            }
            choices.push(StatusLineChoice::custom(
                custom.id.clone(),
                custom.title.clone(),
                custom.description.clone(),
                custom.render_line(CustomStatusContext::Preview { cwd }),
            ));
        }
        choices
    }
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            ...
            StatusSurfacePreviewItem::LaunchProfile => StatusLineItem::LaunchProfile,
            StatusSurfacePreviewItem::LaunchRuntime => StatusLineItem::LaunchRuntime,
...
fn launch_env_status_line_value(key: &str, label: &str) -> Option<String> {
