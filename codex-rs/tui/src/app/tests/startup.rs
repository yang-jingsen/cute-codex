...
async fn later_thread_approval_preserves_input_after_startup_boundary_ends() -> Result<()> {
    ...
    // The composer was just edited, so approval prompts intentionally wait for
    // typing to become idle before opening the protected view.
    std::thread::sleep(crate::bottom_pane::APPROVAL_PROMPT_TYPING_IDLE_DELAY);
