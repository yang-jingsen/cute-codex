...
See the Codex keymap documentation for supported actions and examples."
        ...
        let notify_settings = &app.config.notify_service;
        let usage = app.chat_widget.token_usage();
        if notify_settings
            .notify_service_url
            .as_ref()
            .is_some_and(|url| !url.is_empty())
            && usage.total_tokens >= notify_settings.notify_service_min_tokens
            && crate::notify_service::event_enabled(
                notify_settings,
                codex_config::types::NotifyServiceEvent::SessionExit,
            )
        {
            ...
            let payload = crate::notify_service::build_payload_with_details(
                codex_config::types::NotifyServiceEvent::SessionExit,
                &cwd,
                &notify_settings.notify_service_agent_name,
            ...
            if tokio::time::timeout(
                ...
                crate::notify_service::send_idle_notification_once(
                    notify_settings
                        ...
                        .clone()
                        .unwrap_or_default(),
                    notify_settings.notify_service_token.clone(),
                    payload,
                ),
            )
            .await
            .is_err()
            {
                tracing::warn!("timed out sending session-exit lifecycle notification");
            }
        }
        Ok(AppExitInfo {
