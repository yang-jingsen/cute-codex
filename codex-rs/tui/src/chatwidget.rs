...
pub(crate) struct ChatWidget {
    ...
    cutex_activity_handles: HashMap<String, cutex_activities::CutexActivityHandle>,
    recovered_cutex_activity_handles:
        HashMap<String, cutex_activities::RecoveredCutexActivityHandle>,
    cutex_template_correlations: crate::cutex_template_groups::CutexTemplateCorrelations,
    ...
    // Structured HTTP lifecycle notification state. These fields are kept on
    // the widget so thread replacement and replay can invalidate timers
    // without coupling the app loop to scheduling policy.
    ...
    // Replay reconstructs presentation from persisted events. Keep that path
    // side-effect free even when a lifecycle hook is added later.
    notify_service_replay_suppressed: bool,
    ...
    // Launcher-managed custom status-line catalog, loaded once at construction.
