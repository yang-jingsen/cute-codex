        ...
        match item {
            ...
            ThreadItem::InterAgentMessage {
                ...
                .with_cutex_metadata(
                    ...
                    config.map(|config| &config.tui_cutex_activity.inbound_message),
                )
...
fn fallback_transcript_cell(
    ...
    let default_cutex_presentation = codex_config::types::TuiCutexActivitySettings::default();
    let cutex_presentation = config
        .map(|config| &config.tui_cutex_activity)
        .unwrap_or(&default_cutex_presentation);
    let lines = match item {
        ...
        ThreadItem::ManagedAgentActivity { activity } if activity.phase.is_some() => Vec::new(),
        ...
        | ThreadItem::TaskWatchdogActivity { .. } => {
            crate::cutex_activities::summary_lines(item, cutex_presentation)
        }
