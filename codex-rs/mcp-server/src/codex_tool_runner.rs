...
fn should_forward_event_as_notification(msg: &EventMsg) -> bool {
    !matches!(msg, EventMsg::CutexUiActivity(_))
}
...
async fn run_codex_tool_session_inner(
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                if should_forward_event_as_notification(&event.msg) {
                    outgoing.send_event_as_notification(
                        &event,
                        Some(OutgoingNotificationMeta {
                ...
                match event.msg {
                    ...
                    EventMsg::CutexUiActivity(_) => {
                        // Presentation-only app-server/TUI transport must not become MCP output.
                        continue;
                    }
...
mod tests {
    ...
    #[test]
    fn mcp_notifications_ignore_cutex_ui_activity_and_preserve_ordinary_events() {
        let checkpoint = CutexUiActivityCheckpoint {
            stream_id: "management-v2".to_string(),
            cursor: "cursor-1".to_string(),
            sequence: 1,
        };
        let activity = EventMsg::CutexUiActivity(CutexUiActivityEvent {
            thread_id: ThreadId::new(),
            delivery: CutexUiActivityDelivery {
                schema: CutexUiActivityDeliverySchema::V1,
                class: CutexUiActivityDeliveryClass::Live,
                recovered: false,
                batch_id: "live:management-v2:1:1".to_string(),
                batch_index: 0,
                batch_size: 1,
                source_checkpoint: checkpoint.clone(),
                batch_checkpoint: checkpoint,
            },
            item: TurnItem::TaskWatchdogActivity(TaskWatchdogActivityItem {
                id: "episode-1".to_string(),
                event_id: "event-1".to_string(),
                event_key: "task_watchdog.first_stale".to_string(),
                sequence: 1,
                ...
                attempt_number: 1,
                director_agent_id: "/root".to_string(),
                assignee_agent_id: "/root/worker".to_string(),
                assignee_metadata: None,
                activity_watermark: "watermark-1".to_string(),
                activity_kind: TaskWatchdogActivityKind::StatusProgress,
        ...
        let warning = EventMsg::Warning(WarningEvent {
            message: "ordinary notification".to_string(),
        });

        assert!(!should_forward_event_as_notification(&activity));
        assert!(should_forward_event_as_notification(&warning));
    }
}
