...
struct CapturedSocks5Request {
    destination_host: String,
    destination_port: u16,
    http_request: String,
}

fn spawn_socks5_proxy_listener() -> (
    std::net::SocketAddr,
    std::thread::JoinHandle<CapturedSocks5Request>,
) {
    let listener =
        std::net::TcpListener::bind(("127.0.0.1", 0)).expect("SOCKS5 listener should bind");
    let address = listener
        .local_addr()
        .expect("SOCKS5 listener should have an address");
    let thread = std::thread::spawn(move || {
        let (mut stream, _) = listener.accept().expect("SOCKS5 listener should accept");
        stream
            .set_read_timeout(Some(Duration::from_secs(10)))
            .expect("SOCKS5 stream should get a read timeout");
        stream
            .set_write_timeout(Some(Duration::from_secs(10)))
            .expect("SOCKS5 stream should get a write timeout");

        let mut greeting = [0_u8; 2];
        stream
            .read_exact(&mut greeting)
            .expect("SOCKS5 greeting should read");
        assert_eq!(greeting[0], 5, "SOCKS5 greeting version");
        let mut methods = vec![0_u8; usize::from(greeting[1])];
        stream
            .read_exact(&mut methods)
            .expect("SOCKS5 methods should read");
        assert!(methods.contains(&0), "SOCKS5 client should offer no-auth");
        stream
            .write_all(&[5, 0])
            .expect("SOCKS5 method response should write");

        let mut connect_header = [0_u8; 4];
        stream
            .read_exact(&mut connect_header)
            .expect("SOCKS5 connect header should read");
        assert_eq!(&connect_header[..3], &[5, 1, 0], "SOCKS5 CONNECT header");
        let destination_host = match connect_header[3] {
            1 => {
                let mut address = [0_u8; 4];
                stream
                    .read_exact(&mut address)
                    .expect("SOCKS5 IPv4 destination should read");
            ...
            3 => {
                ...
                stream
                    ...
                    .expect("SOCKS5 hostname length should read");
                let mut hostname = vec![0_u8; usize::from(length[0])];
                stream
                    .read_exact(&mut hostname)
                    .expect("SOCKS5 hostname should read");
                String::from_utf8(hostname).expect("SOCKS5 hostname should be UTF-8")
            }
            4 => {
                let mut address = [0_u8; 16];
                stream
                    .read_exact(&mut address)
                    .expect("SOCKS5 IPv6 destination should read");
                std::net::Ipv6Addr::from(address).to_string()
            }
            address_type => panic!("unsupported SOCKS5 address type {address_type}"),
        };
        let mut port = [0_u8; 2];
        stream
            .read_exact(&mut port)
            .expect("SOCKS5 destination port should read");
        let destination_port = u16::from_be_bytes(port);
        stream
            .write_all(&[5, 0, 0, 1, 0, 0, 0, 0, 0, 0])
            .expect("SOCKS5 success response should write");

        let http_request = read_http_message(&mut stream);
        stream
            .write_all(b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\nConnection: close\r\n\r\nok")
            .expect("SOCKS5 tunneled response should write");

        CapturedSocks5Request {
            destination_host,
            destination_port,
            http_request,
        }
...
#[tokio::test]
async fn route_aware_client_supports_socks5h_with_proxy_side_dns() {
    let (proxy_addr, proxy_thread) = spawn_socks5_proxy_listener();
    let request_url = "http://socks-target.test/proxy-check";
    ...
    let client = factory
        ...
        .expect("SOCKS5h route should build");

    let response = client
        .get(request_url)
        .send()
        .await
        .expect("request should use local SOCKS5h proxy");
    let captured = proxy_thread
        .join()
        .expect("SOCKS5h proxy thread should finish");

    assert_eq!(response.status(), reqwest::StatusCode::OK);
    assert_eq!(captured.destination_host, "socks-target.test");
    assert_eq!(captured.destination_port, 80);
    assert_eq!(
        captured.http_request.lines().next(),
        Some("GET /proxy-check HTTP/1.1")
    );
}
