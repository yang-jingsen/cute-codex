...
#[test]
fn auth_file_env_selects_login_write_target() -> Result<()> {
    let codex_home = TempDir::new()?;
    write_file_auth_config(codex_home.path())?;
    let selected_auth = codex_home.path().join("profiles/work-auth.json");

    let mut cmd = codex_command(codex_home.path())?;
    cmd.env("CODEX_AUTH_FILE", " profiles/work-auth.json ")
        .args([
            "-c",
            "forced_login_method=\"api\"",
            "login",
        ...
        .write_stdin("sk-profile\n")
        .assert()
        .success()
        .stderr(contains("Successfully logged in"));

    let auth: Value = serde_json::from_str(&std::fs::read_to_string(&selected_auth)?)?;
    assert_eq!(auth["OPENAI_API_KEY"], "sk-profile");
