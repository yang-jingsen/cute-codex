...
#[tokio::test]
async fn config_file_env_selects_mcp_read_and_write_target() -> Result<()> {
    let codex_home = TempDir::new()?;
    let default_config = codex_home.path().join("config.toml");
    let selected_config = codex_home.path().join("profiles/work.toml");
    std::fs::create_dir_all(
        selected_config
            .parent()
            .expect("selected config should have a parent"),
    )?;
    let default_contents = "model = \"default-only\"\n";
    std::fs::write(&default_config, default_contents)?;
    std::fs::write(
        &selected_config,
        r#"[mcp_servers.selected]
command = "echo"
args = ["selected"]
"#,
    )?;
    ...
    list_cmd
        .env("CODEX_CONFIG_FILE", " profiles/work.toml ")
        .args(["mcp", "list"])
        .assert()
        .success()
    ...
    add_cmd
        .env("CODEX_CONFIG_FILE", "profiles/work.toml")
        .args(["mcp", "add", "added", "--", "echo", "added"])
        .assert()
        .success()
        .stdout(contains("Added global MCP server 'added'."));

    let selected: toml::Value = toml::from_str(&std::fs::read_to_string(&selected_config)?)?;
    let selected_servers = selected
        .get("mcp_servers")
        .and_then(toml::Value::as_table)
        .expect("selected config should contain MCP servers");
    assert_eq!(
        selected_servers.keys().cloned().collect::<Vec<_>>(),
        vec!["selected".to_string(), "added".to_string()]
    );
    assert_eq!(std::fs::read_to_string(default_config)?, default_contents);
