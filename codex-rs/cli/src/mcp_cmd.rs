...
use codex_core::config::load_global_mcp_servers_with_overrides;
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "cute-codex mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(
    ...
    let config = load_mcp_config(config_overrides, loader_overrides).await?;
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `cute-codex mcp login {name}` to login."
        ),
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `cute-codex mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: cute-codex mcp remove {}", get_args.name);
