...
const CUTEX_CODEX_COMMAND: &str = "cute-codex";
...
#[clap(
    ...
    // `cute-codex-x86_64-unknown-linux-musl`, but the help output should
    // always use the generic launcher name users run.
    bin_name = "cute-codex",
    override_usage = "cute-codex [OPTIONS] [PROMPT]\n       cute-codex [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum LegacyResumeCwdPolicy {
    Current,
}

impl LegacyResumeCwdPolicy {
    const fn config_override(self) -> &'static str {
        match self {
            Self::Current => r#"tui.resume_cwd="current""#,
        }
    }
}
...
struct ResumeCommand {
    ...
    /// Compatibility alias for Cutex-managed resumes; native 0.146 owns the behavior.
    #[arg(long = "cwd-policy", value_enum, hide = true)]
    cwd_policy: Option<LegacyResumeCwdPolicy>,
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | cute-codex login --with-api-key`)"
    )]
    with_api_key: bool,
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Resume(ResumeCommand {
            ...
            let SessionTuiCli(mut config_overrides) = config_overrides;
            apply_legacy_resume_cwd_policy(&mut config_overrides, cwd_policy);
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | cute-codex login --with-api-key`."
                        );
                        std::process::exit(1);
...
fn maybe_print_under_development_feature_warning(codex_home: &std::path::Path, feature: &str) {
    ...
    let config_path = codex_config::resolve_user_config_path(codex_home);
    eprintln!(
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `{CUTEX_CODEX_COMMAND} {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `{CUTEX_CODEX_COMMAND} {subcommand}`"
        );
    }
    Ok(())
}
...
/// Build the final `TuiCli` for a `cute-codex resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `cute-codex` without additional flags.
...
fn apply_legacy_resume_cwd_policy(
    resume_cli: &mut TuiCli,
    cwd_policy: Option<LegacyResumeCwdPolicy>,
) {
    // The retired CLI policy maps directly to the native 0.146 resume setting.
    if let Some(cwd_policy) = cwd_policy {
        resume_cli
            .config_overrides
            .raw_overrides
            .push(cwd_policy.config_override().to_string());
    }
}

/// Build the final `TuiCli` for a `cute-codex fork` invocation.
...
mod tests {
    ...
    fn finalize_resume_from_args(args: &[&str]) -> TuiCli {
        ...
        let SessionTuiCli(mut resume_cli) = resume_cli;
        apply_legacy_resume_cwd_policy(&mut resume_cli, cwd_policy);
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&[CUTEX_CODEX_COMMAND, "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: cute-codex plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: cute-codex plugin marketplace add"),
            ("list", "Usage: cute-codex plugin marketplace list"),
            ("upgrade", "Usage: cute-codex plugin marketplace upgrade"),
            ("remove", "Usage: cute-codex plugin marketplace remove"),
        ] {
            let help = help_from_args(&[
                CUTEX_CODEX_COMMAND,
                "plugin",
                "marketplace",
                subcommand,
    ...
    #[test]
    fn cutex_managed_remote_resume_argv_maps_current_cwd_policy() {
        let args = [
            "cute-codex",
            "--model",
            "gpt-5.6-sol",
            ...
            "--cwd-policy",
            "current",
            "019fac9a-9f14-7c61-adb4-92eefdb6d88c",
            "--remote",
            "unix:///run/cutex/app.sock",
        ];
        let cli = MultitoolCli::try_parse_from(args).expect("managed resume argv should parse");
        let Subcommand::Resume(ResumeCommand {
            cwd_policy, remote, ..
        ...
        assert_eq!(cwd_policy, Some(LegacyResumeCwdPolicy::Current));
        assert_eq!(remote.remote.as_deref(), Some("unix:///run/cutex/app.sock"));

        let interactive = finalize_resume_from_args(&args);
        assert_eq!(
            interactive.resume_session_id.as_deref(),
            Some("019fac9a-9f14-7c61-adb4-92eefdb6d88c")
        );
        assert_eq!(
            interactive.cwd.as_deref(),
            Some(std::path::Path::new("/srv/cutex-session"))
        );
        assert_eq!(
            interactive
                .config_overrides
