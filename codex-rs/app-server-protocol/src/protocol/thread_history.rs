...
impl ThreadHistoryBuilder {
    ...
    fn handle_materialized_item_lifecycle(
        ...
        let should_upsert = match item {
            ...
            | codex_protocol::items::TurnItem::InterAgentMessage(_)
            ...
            | codex_protocol::items::TurnItem::ManagedAgentActivity(_)
            | codex_protocol::items::TurnItem::OutboundInterAgentMessage(_)
            | codex_protocol::items::TurnItem::TaskAssignmentActivity(_)
            | codex_protocol::items::TurnItem::TaskWatchdogActivity(_)
        ...
        if should_upsert {
            ...
            } else if is_cutex_activity {
                self.upsert_late_item_in_turn_id(turn_id, item);
            } else {
    ...
    /// Cutex managed-Agent and Task Service updates may arrive after the Director turn finished.
    /// The live builder intentionally retains only its active turn, so forward an explicit
    /// turn-scoped change to the durable store even when that completed turn is no longer cached.
    /// Full replay builders still take the ordinary in-memory upsert path.
    fn upsert_late_item_in_turn_id(&mut self, turn_id: &str, item: ThreadItem) {
        ...
        if turn_is_known {
            self.upsert_item_in_turn_id(turn_id, item);
        } else if self.is_tracking_changes() {
