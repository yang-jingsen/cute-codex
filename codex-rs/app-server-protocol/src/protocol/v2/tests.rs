...
#[test]
fn thread_inter_agent_message_preserves_request_and_receipt_contract() {
    let params: ThreadInterAgentMessageParams = serde_json::from_value(json!({
        "threadId": "thread_123",
        "messageId": "message_123",
        "author": "/root/sender",
        "recipient": "/root/recipient",
        "otherRecipients": ["/root/observer"],
        "content": "hello",
        "deliveryMode": "after_turn"
    }))
    .expect("params should deserialize");

    let communication = params
        .to_core_communication()
        .expect("params should convert to core communication");
    assert_eq!(communication.id.as_deref(), Some("message_123"));
    assert_eq!(communication.author.as_str(), "/root/sender");
    assert_eq!(communication.recipient.as_str(), "/root/recipient");
    assert_eq!(
        communication
            .other_recipients
            .iter()
    ...
    assert_eq!(
        communication.delivery_mode,
        Some(codex_protocol::protocol::InterAgentDeliveryMode::AfterTurn)
    );
    assert!(communication.trigger_turn);

    let serialized = serde_json::to_value(&params).expect("params should serialize");
    assert_eq!(serialized["messageId"], "message_123");
    assert_eq!(serialized["deliveryMode"], "after_turn");
    assert!(serialized.get("triggerTurn").is_none());
    assert_eq!(
        serde_json::to_value(ThreadInterAgentMessageResponse {
            submission_id: "submission_123".to_string(),
        })
        .expect("response should serialize"),
        json!({ "submissionId": "submission_123" })
    );
}

#[test]
fn thread_inter_agent_message_metadata_is_typed_live_only_presentation() {
    let params: ThreadInterAgentMessageParams = serde_json::from_value(json!({
        "threadId": "thread_123",
        "messageId": "message_123",
        "author": "/root/sender",
        ...
        "deliveryMode": "soon",
        "authorMetadata": {
            "displayName": "Agent 已取餐",
            "cutexSessionId": "cutex.sender.1",
            "profile": "worker",
            "model": "gpt-5",
            "reasoning": "high",
            "role": "Worker",
        ...
        "recipientMetadata": {"displayName": "Director"}
    }))
    .expect("typed presentation should deserialize");

    let communication = params
        .to_core_communication()
        .expect("typed presentation should validate");
    assert_eq!(
        communication
            .author_metadata
            .as_ref()
    ...
    let model_item = communication.to_model_input_item();
    let codex_protocol::models::ResponseItem::AgentMessage {
        author,
        recipient,
        content,
        ..
        ...
        panic!("expected canonical AgentMessage");
    };
    assert_eq!(author, "/root/sender");
    assert_eq!(recipient, "/root/recipient");
    assert_eq!(
        content,
        vec![
            codex_protocol::models::AgentMessageInputContent::InputText {
                text: "canonical message".to_string(),
            }
        ]
    );
    let serialized = serde_json::to_string(&communication).expect("communication should serialize");
    assert!(!serialized.contains("Agent 已取餐"));
    assert!(!serialized.contains("cutex.sender.1"));

    let malformed: ThreadInterAgentMessageParams = serde_json::from_value(json!({
        "threadId": "thread_123",
        "messageId": "message_123",
        "author": "/root/sender",
        "recipient": "/root/recipient",
...
fn thread_inter_agent_message_task_service_projection_is_model_only_and_preserves_wake_mode() {
    ...
    let communication = params
        .to_core_communication()
        .expect("params should convert to core communication");
    assert_eq!(communication.content, raw);
    assert_eq!(
        communication.model_visible_content.as_deref(),
        Some(
    ...
    assert_eq!(
        communication.delivery_mode,
        Some(codex_protocol::protocol::InterAgentDeliveryMode::Soon)
    );
    assert!(communication.trigger_turn);
    let model_item = communication.to_model_input_item();
    let codex_protocol::models::ResponseItem::AgentMessage { content, .. } = model_item else {
        panic!("Task Service communication must retain native agent-message semantics");
    };
    let fallback_content = codex_protocol::models::plaintext_agent_message_content(&content)
        .expect("plaintext model projection");
    assert_eq!(
        Some(fallback_content.as_str()),
        communication.model_visible_content.as_deref()
    );
    assert!(!fallback_content.contains("Project ID:"));

    let serialized = serde_json::to_value(&communication).expect("communication should serialize");
    assert_eq!(serialized["content"], raw);
    assert!(serialized["content"].as_str().is_some_and(|content| {
        content.contains("notification-01") && content.contains("message-01")
    }));
    assert!(serialized.get("model_visible_content").is_none());
}

#[test]
fn thread_inter_agent_message_task_service_presentation_is_typed_live_only() {
    ...
    let communication = params.to_core_communication().expect("typed ingress");
    let presentation = communication
        .task_service_presentation
        .as_deref()
        .expect("authenticated Task Service presentation");
    assert_eq!(presentation.class, TaskServiceMessageClass::Assignment);
    assert_eq!(presentation.task_name, "worker-task");
    assert_eq!(presentation.assignment_id, "assignment-01");
    assert_eq!(presentation.project_id.as_deref(), Some("cutex-stack-main"));
    assert_eq!(presentation.semantic_payload, "Do this exactly once.\n");
    let serialized = serde_json::to_string(&communication).expect("serialize canonical ingress");
    assert!(!serialized.contains("task_service_presentation"));
    assert!(
        serialized.contains("notification-01"),
        "raw envelope remains durable"
    );
}

#[test]
fn thread_inter_agent_message_defaults_omitted_other_recipients() {
    let params: ThreadInterAgentMessageParams = serde_json::from_value(json!({
        "threadId": "thread_123",
        "messageId": "message_123",
        "author": "/root/sender",
    ...
    assert_eq!(params.other_recipients, None);
    assert!(
        params
            .to_core_communication()
...
#[test]
fn thread_inter_agent_message_requires_delivery_mode() {
    let error = serde_json::from_value::<ThreadInterAgentMessageParams>(json!({
        "threadId": "thread_123",
        "messageId": "message_123",
        "author": "/root/sender",
        "recipient": "/root/recipient",
    ...
    .expect_err("deliveryMode should be required");

    assert!(error.to_string().contains("deliveryMode"));
}

#[test]
fn thread_inter_agent_message_rejects_invalid_identity_fields() {
    let empty_id = ThreadInterAgentMessageParams {
        thread_id: "thread_123".to_string(),
    ...
    let invalid_author = ThreadInterAgentMessageParams {
        message_id: "message_123".to_string(),
    ...
    assert!(
        invalid_author
            .to_core_communication()
            .expect_err("relative author should fail")
            .contains("absolute agent paths")
    );
}

#[test]
fn thread_inter_agent_message_status_preserves_exact_tagged_wire_shape() {
    let response = ThreadInterAgentMessageStatusResponse {
        schema: "cutex/inter-agent-delivery-status/v1".to_string(),
        thread_id: "thread_123".to_string(),
        statuses: vec![
            InterAgentDeliveryStatus::Pending {
                message_id: "message_pending".to_string(),
                semantic_sha256: "a".repeat(64),
            },
            InterAgentDeliveryStatus::ContextPersisted {
                message_id: "message_a4".to_string(),
                semantic_sha256: "b".repeat(64),
                receipt: InterAgentDeliveryReceipt {
                    receipt_id: "a4r_receipt".to_string(),
                    response_item_id: "amsg_item".to_string(),
    ...
    assert_eq!(
        serde_json::to_value(response).expect("serialize status response"),
        json!({
            "schema": "cutex/inter-agent-delivery-status/v1",
            "threadId": "thread_123",
            "statuses": [
                {
                    "state": "pending",
                ...
                {
                    "state": "context_persisted",
                    "messageId": "message_a4",
                    "semanticSha256": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
                    "receipt": {
                        "receiptId": "a4r_receipt",
                        "responseItemId": "amsg_item",
                        "turnId": "turn_123",
                        "rolloutOrdinal": 7
                    }
                }
...
#[test]
fn thread_inter_agent_message_status_params_reject_additive_fields() {
    for value in [
        json!({
            "threadId": "thread_123",
            "messages": [{"messageId": "message_123", "semanticSha256": "a".repeat(64)}],
            "extra": true
        }),
        json!({
            "threadId": "thread_123",
        ...
        serde_json::from_value::<ThreadInterAgentMessageStatusParams>(value)
            .expect_err("unknown status request fields must fail closed");
    }
}

#[test]
fn thread_cutex_activity_is_strictly_typed_and_bounded() {
    let params: ThreadCutexActivityParams = serde_json::from_value(json!({
        "threadId": "thread_123",
        "delivery": cutex_delivery_json(7),
        "activity": {
            "type": "outboundInterAgentMessage",
            "id": "message_123",
            "eventId": "event_123",
            "sequence": 7,
            "occurredAtMs": 1725000123456_i64,
            "senderAgentId": "director_123",
            "senderAgentName": "Director",
            "senderMetadata": {"displayName": "Director", "role": "Director"},
            "recipientAgentId": "worker_123",
            "recipientAgentName": "Worker",
            "recipientMetadata": {"displayName": "Agent 已取餐", "profile": "worker"},
            "otherRecipientAgentIds": [],
            "deliveryMode": "after_turn",
            "status": "sent",
            "contentPreview": "focused task",
            "detail": null
        }
    }))
    .expect("typed Cutex activity should deserialize");
    params
        .validate()
        .expect("valid activity should pass bounds");
    assert!(matches!(
        &params.activity,
        CutexUiActivity::OutboundInterAgentMessage(OutboundInterAgentMessageItem {
            sequence: 7,
            occurred_at_ms: 1_725_000_123_456,
            recipient_metadata: Some(CutexParticipantPresentation {
    ...
    let CutexUiActivity::OutboundInterAgentMessage(activity) = &mut invalid.activity else {
        unreachable!()
    };
    activity.sequence = 0;
    assert_eq!(
        invalid.validate().expect_err("zero sequence should fail"),
        "sequence must be greater than zero"
    );

    let untyped = serde_json::from_value::<ThreadCutexActivityParams>(json!({
        "threadId": "thread_123",
        "delivery": cutex_delivery_json(1),
        "activity": { "type": "freeText", "content": "do not parse me" }
    }));
    assert!(untyped.is_err(), "free-form fallback must not be accepted");

    let omitted_turn: ThreadCutexActivityParams = serde_json::from_value(json!({
        "threadId": "thread_123",
        "delivery": cutex_delivery_json(1),
        "activity": {
            ...
            "operation": "create",
            "status": "inProgress",
            "managedAgentId": "worker_123"
        }
    }))
    .expect("closed UI-only params should deserialize");
    omitted_turn
        .validate()
        .expect("an omitted turnId should use the UI-only lane");
    let valid_wire = serde_json::to_value(&omitted_turn).unwrap();
    let mut with_turn = valid_wire.clone();
    with_turn["turnId"] = json!("native-turn");
    assert!(serde_json::from_value::<ThreadCutexActivityParams>(with_turn).is_err());

    let mut missing_delivery = valid_wire.clone();
    missing_delivery.as_object_mut().unwrap().remove("delivery");
    assert!(serde_json::from_value::<ThreadCutexActivityParams>(missing_delivery).is_err());

    for (field, value) in [
        ("schema", json!("cutex/ui-activity-delivery/v2")),
        ("class", json!("historical")),
        ("batchSize", json!(0)),
        ("batchSize", json!(33)),
        ("batchIndex", json!(1)),
    ] {
        let mut invalid = valid_wire.clone();
        invalid["delivery"][field] = value;
        let result = serde_json::from_value::<ThreadCutexActivityParams>(invalid)
            .map_err(|error| error.to_string())
            .and_then(|params| params.validate());
        assert!(result.is_err(), "invalid delivery {field} must fail closed");
    }

    let mut unknown_delivery = valid_wire.clone();
    unknown_delivery["delivery"]["priority"] = json!("high");
    assert!(serde_json::from_value::<ThreadCutexActivityParams>(unknown_delivery).is_err());

    let mut mismatched_stream = valid_wire;
    mismatched_stream["delivery"]["batchCheckpoint"]["streamId"] = json!("other-stream");
    assert_eq!(
        serde_json::from_value::<ThreadCutexActivityParams>(mismatched_stream)
            .unwrap()
            .validate()
            .unwrap_err(),
        "delivery checkpoints must use the same streamId"
    );
}

#[test]
fn task_watchdog_activity_matches_frozen_wire_and_fails_closed() {
    let wire = json!({
        "threadId": "thread_123",
        "delivery": cutex_delivery_json(44),
        "activity": {
            "type": "taskWatchdogActivity",
            "id": "episode-1",
            "eventId": "fact-1",
            "eventKey": "task_watchdog.first_stale",
            "sequence": 44,
            "occurredAtMs": 1787879400000_i64,
            "projectId": "project-1",
            "taskId": "task-1",
            "taskRevision": 3,
            "assignmentId": "assignment-1",
            "attemptNumber": 2,
            "directorAgentId": "cutex.director",
            "assigneeAgentId": "cutex.worker",
            "assigneeMetadata": {
                "displayName": "Worker",
                "profile": "worker",
                "model": "gpt-5.6",
            ...
            "activityWatermark": "2026-08-28T01:00:00Z",
            "activityKind": "last_tool_call",
            "idleDurationSecs": 600,
            "stage": "first_stale",
            "sourceSequence": 41
        }
    });
    let params: ThreadCutexActivityParams =
        serde_json::from_value(wire.clone()).expect("frozen watchdog wire");
    params.validate().expect("valid watchdog activity");
    assert!(matches!(
        params.activity,
    ...
    for (field, value) in [
        ("eventKey", json!("task_watchdog.director_escalated")),
        ("stage", json!("recovered")),
        ("taskRevision", json!(0)),
        ("attemptNumber", json!(9_007_199_254_740_992_u64)),
        ("id", json!("x".repeat(257))),
        ("eventId", json!("x".repeat(257))),
        ("projectId", json!("x".repeat(257))),
        ("projectId", json!("")),
        ("taskId", json!("x".repeat(257))),
        ("assignmentId", json!("x".repeat(257))),
        ("assigneeAgentId", json!("x".repeat(257))),
        ("activityWatermark", json!("2026-08-28 01:00:00")),
    ] {
        let mut invalid = wire.clone();
        invalid["activity"][field] = value;
        let result = serde_json::from_value::<ThreadCutexActivityParams>(invalid)
            .map_err(|error| error.to_string())
            .and_then(|params| params.validate());
    ...
    unknown["activity"]["content"] = json!("Continue the task");
    assert!(serde_json::from_value::<ThreadCutexActivityParams>(unknown).is_err());

    let mut with_turn = wire;
    with_turn["turnId"] = json!("native-turn");
    assert!(serde_json::from_value::<ThreadCutexActivityParams>(with_turn).is_err());
}

#[test]
fn managed_agent_phase_activity_matches_the_frozen_wire_and_truth_rules() {
    let params: ThreadCutexActivityParams = serde_json::from_value(json!({
        "threadId": "thread_123",
        "delivery": cutex_delivery_json(44),
        "activity": {
            "type": "managedAgentActivity",
            "id": "rotate-action-1",
            "eventId": "management-envelope-44",
            "sequence": 44,
            "occurredAtMs": 1725000123456_i64,
            "projectId": "project-1",
            "operation": "directorRotate",
            "status": "inProgress",
            "actionId": "rotate-action-1",
            "phaseEventId": "agent-management:rotate-action-1:phase:3",
            "phase": "predecessorClosing",
            "managedAgentId": "cutex.predecessor",
            "managedAgentName": "Old Director",
            "managedAgentMetadata": {"displayName": "Old Director", "role": "Director"},
            "predecessorAgentId": "cutex.predecessor",
            "predecessorAgentName": "Old Director",
            "predecessorMetadata": {"cutexSessionId": "cutex.predecessor"},
            "successorAgentId": "cutex.successor",
            "successorAgentName": "New Director",
            "successorMetadata": {"displayName": "New Director"},
            "replacePolicy": null,
            "rotationMode": "closePredecessorThenCreateWithMessage",
            "authorityEpoch": null
        }
    }))
    .expect("frozen Agent Management phase should deserialize");
    params.validate().expect("frozen phase should validate");
    let CutexUiActivity::ManagedAgentActivity(activity) = &params.activity else {
        panic!("expected managed agent activity");
    };
    assert_eq!(activity.project_id.as_deref(), Some("project-1"));
    assert_eq!(activity.action_id.as_deref(), Some("rotate-action-1"));
    assert_eq!(
        activity.phase_event_id.as_deref(),
        Some("agent-management:rotate-action-1:phase:3")
    );
    assert_eq!(
        activity.phase,
        Some(AgentManagementPhase::PredecessorClosing)
    );
    assert_eq!(
        activity.rotation_mode,
        Some(AgentManagementRotationMode::ClosePredecessorThenCreateWithMessage)
    );

    let base = params;
    for expected in [
        "retain-predecessor rotation cannot publish a predecessor close phase",
        "bootstrap-only rotation cannot publish a message phase",
        "post-transfer Director rotation phase requires authorityEpoch",
        "phase status must match the authoritative phase mapping",
        "actionId must equal the stable activity id",
        "phaseEventId must use the authoritative Agent Management identity",
    ] {
        let mut invalid = base.clone();
        let CutexUiActivity::ManagedAgentActivity(activity) = &mut invalid.activity else {
            unreachable!()
        };
        match expected {
            "retain-predecessor rotation cannot publish a predecessor close phase" => {
                activity.rotation_mode =
                    Some(AgentManagementRotationMode::RetainPredecessorWithMessage);
            }
            "bootstrap-only rotation cannot publish a message phase" => {
                activity.phase = Some(AgentManagementPhase::MessagePending);
                activity.rotation_mode =
                    Some(AgentManagementRotationMode::RetainPredecessorBootstrapOnly);
            }
            "post-transfer Director rotation phase requires authorityEpoch" => {
                activity.phase = Some(AgentManagementPhase::SuccessorReady);
            }
            "phase status must match the authoritative phase mapping" => {
                activity.status = ManagedAgentActivityStatus::Completed;
            }
            "actionId must equal the stable activity id" => {
                activity.action_id = Some("other-action".into());
            }
            "phaseEventId must use the authoritative Agent Management identity" => {
                activity.phase_event_id = Some("invented-event".into());
            }
            _ => unreachable!(),
        }
        assert_eq!(
            invalid.validate().expect_err("invalid phase must fail"),
            expected
        );
    }

    let mut legacy = base;
    let CutexUiActivity::ManagedAgentActivity(activity) = &mut legacy.activity else {
        unreachable!()
    };
    activity.phase = None;
    assert_eq!(
        legacy
            .validate()
            .expect_err("auxiliary phase fields without a phase must fail"),
        "Agent Management phase fields require phase"
    );
}

fn cutex_delivery_json(sequence: u64) -> serde_json::Value {
    json!({
        "schema": "cutex/ui-activity-delivery/v1",
        "class": "live",
        "recovered": false,
        "batchId": format!("live:management-v2:{sequence}:{sequence}"),
        "batchIndex": 0,
        "batchSize": 1,
        "sourceCheckpoint": {
            "streamId": "management-v2",
