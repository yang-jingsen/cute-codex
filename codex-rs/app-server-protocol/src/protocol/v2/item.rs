...
pub use codex_protocol::items::AgentManagementPhase;
pub use codex_protocol::items::AgentManagementReplacePolicy;
...
pub use codex_protocol::items::CutexParticipantPresentation;
pub use codex_protocol::items::CutexUiActivity;
pub use codex_protocol::items::CutexUiActivityCheckpoint;
pub use codex_protocol::items::CutexUiActivityDelivery;
pub use codex_protocol::items::CutexUiActivityDeliveryClass;
pub use codex_protocol::items::CutexUiActivityDeliverySchema;
use codex_protocol::items::DynamicToolCallStatus as CoreDynamicToolCallStatus;
pub use codex_protocol::items::ManagedAgentActivityItem;
pub use codex_protocol::items::ManagedAgentActivityStatus;
pub use codex_protocol::items::ManagedAgentOperation;
use codex_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
pub use codex_protocol::items::OutboundInterAgentMessageItem;
pub use codex_protocol::items::OutboundInterAgentMessageStatus;
pub use codex_protocol::items::TaskAssignmentActivityItem;
pub use codex_protocol::items::TaskAssignmentActivityStatus;
pub use codex_protocol::items::TaskServiceMessageClass;
pub use codex_protocol::items::TaskServiceMessagePresentation;
pub use codex_protocol::items::TaskWatchdogActivityItem;
pub use codex_protocol::items::TaskWatchdogActivityKind;
pub use codex_protocol::items::TaskWatchdogStage;
...
pub use codex_protocol::protocol::InterAgentDeliveryMode;
...
pub enum ThreadItem {
    ...
    InterAgentMessage {
        ...
        #[ts(optional)]
        author_metadata: Option<CutexParticipantPresentation>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        #[ts(optional)]
        recipient_metadata: Option<CutexParticipantPresentation>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        #[ts(optional)]
    ...
    /// A durable Cutex managed-Agent lifecycle item. This is presentation state only and does not
    /// imply native parent/child ownership.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    ManagedAgentActivity {
        activity: ManagedAgentActivityItem,
    },
    /// A sender-side Cutex Agent Bus activity. The content is a bounded preview and is never
    /// appended to the sender model context by this item.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    OutboundInterAgentMessage {
        activity: OutboundInterAgentMessageItem,
    },
    /// Typed Cutex Task Service timeline state. Consumers should requery the authoritative task
    /// snapshot when sequence changes rather than treating this watch item as authority.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    TaskAssignmentActivity {
        activity: TaskAssignmentActivityItem,
    },
    /// Live, presentation-only stale-running Task Service episode metadata.
    #[serde(rename_all = "camelCase")]
    #[ts(rename_all = "camelCase")]
    TaskWatchdogActivity {
...
impl ThreadItem {
    pub fn id(&self) -> &str {
        match self {
            ...
            ThreadItem::ManagedAgentActivity { activity } => &activity.id,
            ThreadItem::OutboundInterAgentMessage { activity } => &activity.id,
            ThreadItem::TaskAssignmentActivity { activity } => &activity.id,
            ThreadItem::TaskWatchdogActivity { activity } => &activity.id,
    ...
    fn from(value: CoreTurnItem) -> Self {
        match value {
            ...
            CoreTurnItem::InterAgentMessage(message) => {
                let task_service_presentation = message.task_service_presentation.or_else(|| {
                    task_service_message_presentation(&message.author, &message.content)
                });
                ThreadItem::InterAgentMessage {
                    id: message.id,
                    author: message.author.to_string(),
                    recipient: message.recipient.to_string(),
                    other_recipients: message
                        .other_recipients
                        .into_iter()
                        .map(String::from)
                        .collect(),
                    ...
                    author_metadata: message.author_metadata,
            ...
            CoreTurnItem::ManagedAgentActivity(activity) => {
                ThreadItem::ManagedAgentActivity { activity }
            }
            CoreTurnItem::OutboundInterAgentMessage(activity) => {
                ThreadItem::OutboundInterAgentMessage { activity }
            }
            CoreTurnItem::TaskAssignmentActivity(activity) => {
                ThreadItem::TaskAssignmentActivity { activity }
            }
            CoreTurnItem::TaskWatchdogActivity(activity) => {
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct CutexUiActivityNotification {
    pub thread_id: String,
