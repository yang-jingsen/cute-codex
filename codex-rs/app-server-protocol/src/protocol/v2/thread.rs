...
use super::CutexParticipantPresentation;
use super::CutexUiActivity;
use super::CutexUiActivityDelivery;
use super::CutexUiActivityDeliveryClass;
...
use crate::TS;
use crate::protocol::task_service_model_projection::project_task_service_model_content;
use crate::protocol::task_service_model_projection::task_service_message_presentation;
...
use codex_protocol::items::AgentManagementRotationMode;
...
pub struct ThreadInterAgentMessageParams {
    ...
    #[ts(optional = nullable)]
    pub other_recipients: Option<Vec<String>>,
    pub content: String,
...
impl ThreadInterAgentMessageParams {
    pub fn to_core_communication(&self) -> Result<InterAgentCommunication, String> {
        ...
        communication.model_visible_content =
            project_task_service_model_content(&communication.author, &communication.content);
        communication.task_service_presentation =
            task_service_message_presentation(&communication.author, &communication.content)
                .map(Box::new);
        validate_participant_metadata("authorMetadata", self.author_metadata.as_ref())?;
        validate_participant_metadata("recipientMetadata", self.recipient_metadata.as_ref())?;
        communication.author_metadata = self.author_metadata.clone().map(Box::new);
        communication.recipient_metadata = self.recipient_metadata.clone().map(Box::new);
        communication.id = Some(ResponseItemId::from_server(self.message_id.clone()));
        Ok(communication)
    }
}
...
pub struct ThreadInterAgentMessageStatusParams {
    pub thread_id: String,
    pub messages: Vec<InterAgentDeliveryStatusQuery>,
}
...
#[schemars(deny_unknown_fields)]
#[ts(export_to = "v2/")]
pub struct InterAgentDeliveryStatusQuery {
    pub message_id: String,
...
pub struct ThreadInterAgentMessageStatusResponse {
    pub schema: String,
    pub thread_id: String,
    pub statuses: Vec<InterAgentDeliveryStatus>,
}
...
#[ts(tag = "state", rename_all = "snake_case", export_to = "v2/")]
pub enum InterAgentDeliveryStatus {
    Unknown {
        #[ts(rename = "messageId")]
        message_id: String,
        #[ts(rename = "semanticSha256")]
...
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub struct InterAgentDeliveryReceipt {
...
impl ThreadCutexActivityParams {
    pub fn validate(&self) -> Result<(), String> {
        validate_cutex_ui_activity_delivery(&self.delivery)?;
        match &self.activity {
            CutexUiActivity::ManagedAgentActivity(item) => {
                ...
                validate_nonempty_bounded("managedAgentId", &item.managed_agent_id, 512)?;
                validate_optional_bounded(
                    "managedAgentName",
                    item.managed_agent_name.as_deref(),
                    512,
                )?;
                validate_participant_metadata(
                    "managedAgentMetadata",
                    item.managed_agent_metadata.as_ref(),
                )?;
                validate_optional_bounded("actionId", item.action_id.as_deref(), 512)?;
                validate_optional_bounded("phaseEventId", item.phase_event_id.as_deref(), 1_024)?;
                validate_optional_bounded(
                    "predecessorAgentId",
                    item.predecessor_agent_id.as_deref(),
                    512,
                )?;
                validate_optional_bounded(
                    "predecessorAgentName",
                    item.predecessor_agent_name.as_deref(),
                    512,
                )?;
                validate_participant_metadata(
                    "predecessorMetadata",
                    item.predecessor_metadata.as_ref(),
                )?;
                validate_optional_bounded(
                    "successorAgentId",
                    item.successor_agent_id.as_deref(),
                    512,
                )?;
                validate_optional_bounded(
                    "successorAgentName",
                    item.successor_agent_name.as_deref(),
                    512,
                )?;
                validate_participant_metadata(
                    "successorMetadata",
                    item.successor_metadata.as_ref(),
                )?;
                validate_agent_management_phase(item)?;
                validate_optional_bounded(
                    "managedAgentRole",
                    item.managed_agent_role.as_deref(),
                    256,
                )?;
                validate_optional_bounded(
                    "initialTaskPreview",
                    item.initial_task_preview.as_deref(),
                    2_000,
                )?;
                validate_optional_bounded("detail", item.detail.as_deref(), 2_000)?;
            }
            CutexUiActivity::OutboundInterAgentMessage(item) => {
                ...
                validate_nonempty_bounded("senderAgentId", &item.sender_agent_id, 512)?;
                validate_optional_bounded(
                    "senderAgentName",
                    item.sender_agent_name.as_deref(),
                    512,
                )?;
                validate_participant_metadata("senderMetadata", item.sender_metadata.as_ref())?;
                validate_participant_metadata(
                    "recipientMetadata",
                    item.recipient_metadata.as_ref(),
                )?;
                validate_nonempty_bounded("recipientAgentId", &item.recipient_agent_id, 512)?;
                validate_optional_bounded(
                    "recipientAgentName",
                    item.recipient_agent_name.as_deref(),
                    512,
                )?;
                if item.other_recipient_agent_ids.len() > 32 {
                    return Err(
                        "otherRecipientAgentIds must contain at most 32 entries".to_string()
                    );
                }
                for recipient in &item.other_recipient_agent_ids {
                    validate_nonempty_bounded("otherRecipientAgentIds[]", recipient, 512)?;
                }
                validate_optional_bounded(
                    "contentPreview",
            ...
            CutexUiActivity::TaskAssignmentActivity(item) => {
                ...
                validate_nonempty_bounded("taskId", &item.task_id, 512)?;
                validate_optional_bounded("taskTitle", item.task_title.as_deref(), 1_000)?;
                validate_nonempty_bounded("directorAgentId", &item.director_agent_id, 512)?;
                validate_optional_bounded(
                    "directorAgentName",
                    item.director_agent_name.as_deref(),
                    512,
                )?;
                validate_participant_metadata("directorMetadata", item.director_metadata.as_ref())?;
                validate_participant_metadata("assigneeMetadata", item.assignee_metadata.as_ref())?;
                validate_nonempty_bounded("assigneeAgentId", &item.assignee_agent_id, 512)?;
                validate_optional_bounded(
                    "assigneeAgentName",
                    item.assignee_agent_name.as_deref(),
                    512,
                )?;
                validate_optional_bounded("attemptId", item.attempt_id.as_deref(), 512)?;
                validate_optional_bounded("detail", item.detail.as_deref(), 2_000)?;
            }
            CutexUiActivity::TaskWatchdogActivity(item) => {
                validate_activity_identity(
                    &item.id,
                    &item.event_id,
                    item.sequence,
                ...
                validate_nonempty_bounded("episode id", &item.id, 256)?;
                validate_nonempty_bounded("fact eventId", &item.event_id, 256)?;
                validate_nonempty_bounded("eventKey", &item.event_key, 128)?;
                if item.event_key != item.stage.event_key() {
                    return Err("eventKey must match the watchdog stage".to_string());
                }
                if let Some(project_id) = item.project_id.as_deref() {
                    validate_nonempty_bounded("projectId", project_id, 256)?;
                }
                validate_nonempty_bounded("taskId", &item.task_id, 256)?;
                validate_nonempty_bounded("assignmentId", &item.assignment_id, 256)?;
                validate_nonempty_bounded("directorAgentId", &item.director_agent_id, 512)?;
                validate_nonempty_bounded("assigneeAgentId", &item.assignee_agent_id, 256)?;
                validate_participant_metadata("assigneeMetadata", item.assignee_metadata.as_ref())?;
                validate_nonempty_bounded("activityWatermark", &item.activity_watermark, 128)?;
                chrono::DateTime::parse_from_rfc3339(&item.activity_watermark)
                    .map_err(|_| "activityWatermark must be an RFC3339 date-time".to_string())?;
                const MAX_SAFE_INTEGER: u64 = 9_007_199_254_740_991;
                for (field, value, positive) in [
                    ("sequence", item.sequence, true),
                    ("taskRevision", item.task_revision, true),
                    ("attemptNumber", item.attempt_number, true),
                    ("idleDurationSecs", item.idle_duration_secs, false),
                    ("sourceSequence", item.source_sequence, false),
                ] {
                    if value > MAX_SAFE_INTEGER || (positive && value == 0) {
                        ...
                        return Err(format!("{field} must be {requirement}"));
                    }
                }
            }
        }
...
fn validate_cutex_ui_activity_delivery(delivery: &CutexUiActivityDelivery) -> Result<(), String> {
    const MAX_SAFE_INTEGER: u64 = 9_007_199_254_740_991;
    validate_nonempty_bounded("delivery.batchId", &delivery.batch_id, 1_024)?;
    let class_prefix = match delivery.class {
        CutexUiActivityDeliveryClass::Live => "live:",
        CutexUiActivityDeliveryClass::CatchUp => "catch_up:",
    };
    if !delivery.batch_id.starts_with(class_prefix) {
        return Err("delivery.batchId must start with the delivery class".to_string());
    }
    if delivery.batch_size == 0 || delivery.batch_size > 32 {
        return Err("delivery.batchSize must be between 1 and 32".to_string());
    }
    if delivery.batch_index >= delivery.batch_size {
        return Err("delivery.batchIndex must be less than delivery.batchSize".to_string());
    }
    for (field, checkpoint) in [
        ("delivery.sourceCheckpoint", &delivery.source_checkpoint),
        ("delivery.batchCheckpoint", &delivery.batch_checkpoint),
    ] {
        validate_nonempty_bounded(&format!("{field}.streamId"), &checkpoint.stream_id, 512)?;
        validate_nonempty_bounded(&format!("{field}.cursor"), &checkpoint.cursor, 1_024)?;
        if checkpoint.sequence == 0 || checkpoint.sequence > MAX_SAFE_INTEGER {
            return Err(format!(
                "{field}.sequence must be a positive JavaScript-safe integer"
            ));
        }
    }
    if delivery.source_checkpoint.stream_id != delivery.batch_checkpoint.stream_id {
        return Err("delivery checkpoints must use the same streamId".to_string());
    }
    if delivery.source_checkpoint.sequence > delivery.batch_checkpoint.sequence {
        return Err(
            "delivery.sourceCheckpoint.sequence must not exceed batchCheckpoint.sequence"
                .to_string(),
        );
    }
    Ok(())
...
fn validate_agent_management_phase(item: &ManagedAgentActivityItem) -> Result<(), String> {
    let Some(phase) = item.phase else {
        if item.action_id.is_some()
            || item.phase_event_id.is_some()
            || item.predecessor_agent_id.is_some()
            || item.predecessor_agent_name.is_some()
            || item.predecessor_metadata.is_some()
            || item.successor_agent_id.is_some()
            || item.successor_agent_name.is_some()
            || item.successor_metadata.is_some()
            || item.replace_policy.is_some()
            || item.rotation_mode.is_some()
            || item.authority_epoch.is_some()
        {
            return Err("Agent Management phase fields require phase".to_string());
        }
        return Ok(());
    };
    ...
    let action_id = item
        .action_id
        .as_deref()
        .ok_or_else(|| "phase requires actionId".to_string())?;
    if action_id != item.id {
        return Err("actionId must equal the stable activity id".to_string());
    }
    let phase_event_id = item
        .phase_event_id
        .as_deref()
        .ok_or_else(|| "phase requires phaseEventId".to_string())?;
    let prefix = format!("agent-management:{action_id}:phase:");
    let Some(sequence) = phase_event_id.strip_prefix(&prefix) else {
        return Err(
            "phaseEventId must use the authoritative Agent Management identity".to_string(),
        );
    };
    if sequence
        .parse::<u64>()
        ...
        .is_none()
    {
        return Err("phaseEventId must end in a positive phase sequence".to_string());
    }
    if item.authority_epoch == Some(0) {
        return Err("authorityEpoch must be positive".to_string());
    }
    let expected_status = match phase {
        AgentManagementPhase::Complete => {
            codex_protocol::items::ManagedAgentActivityStatus::Completed
        }
        AgentManagementPhase::NoWrite
        | AgentManagementPhase::OwnerActionRequired
        | AgentManagementPhase::Failure => {
            codex_protocol::items::ManagedAgentActivityStatus::Failed
        }
        _ => codex_protocol::items::ManagedAgentActivityStatus::InProgress,
    };
    if item.status != expected_status {
        return Err("phase status must match the authoritative phase mapping".to_string());
    }

    if matches!(
        item.rotation_mode,
        Some(
            AgentManagementRotationMode::RetainPredecessorWithMessage
                | AgentManagementRotationMode::RetainPredecessorBootstrapOnly
        )
    ) && matches!(
        phase,
        AgentManagementPhase::PredecessorClosing | AgentManagementPhase::PredecessorClosed
    ) {
        return Err(
            "retain-predecessor rotation cannot publish a predecessor close phase".to_string(),
        );
    }
    if matches!(
        item.rotation_mode,
        ...
        phase,
        AgentManagementPhase::MessagePending | AgentManagementPhase::MessageQueued
    ) {
        return Err("bootstrap-only rotation cannot publish a message phase".to_string());
    }
    if matches!(
        phase,
        ...
        item.operation,
        codex_protocol::items::ManagedAgentOperation::DirectorRotate
    ) && item.authority_epoch.is_none()
    {
        return Err("post-transfer Director rotation phase requires authorityEpoch".to_string());
    }
    Ok(())
}

fn validate_participant_metadata(
    field: &str,
    metadata: Option<&CutexParticipantPresentation>,
) -> Result<(), String> {
    let Some(metadata) = metadata else {
        return Ok(());
    };
    for (name, value) in [
        ("displayName", metadata.display_name.as_deref()),
        ("cutexSessionId", metadata.cutex_session_id.as_deref()),
        ("profile", metadata.profile.as_deref()),
        ("model", metadata.model.as_deref()),
        ("reasoning", metadata.reasoning.as_deref()),
        ("role", metadata.role.as_deref()),
        ("runtimeBackend", metadata.runtime_backend.as_deref()),
    ] {
        ...
        {
            return Err(format!(
                "{field}.{name} must be nonempty, bounded, and printable"
            ));
        }
    }
    Ok(())
...
fn validate_activity_identity(
    ...
    validate_nonempty_bounded("activity id", item_id, 512)?;
    validate_nonempty_bounded("eventId", event_id, 512)?;
    if sequence == 0 {
        return Err("sequence must be greater than zero".to_string());
    }
    if occurred_at_ms < 0 {
        return Err("occurredAtMs must not be negative".to_string());
    }
    Ok(())
}
...
impl From<codex_protocol::protocol::CutexUiActivityDisposition>
    for CutexUiActivityIngestionDisposition
{
    fn from(value: codex_protocol::protocol::CutexUiActivityDisposition) -> Self {
        match value {
            codex_protocol::protocol::CutexUiActivityDisposition::Accepted => Self::Accepted,
            codex_protocol::protocol::CutexUiActivityDisposition::Duplicate => Self::Duplicate,
            codex_protocol::protocol::CutexUiActivityDisposition::Stale => Self::Stale,
        }
    }
}
