"""Owned adaptation of S5c model-free persistence barrier proof (Linux x86_64).
...
Native app-server and its children inherit a seccomp ban on creating IPv4/IPv6 sockets.
"""
...
def no_ip_sockets():
    ...
    rules = (Filter * 11)(
        ...
        Filter(0x20, 0, 0, 0),
        Filter(0x54, 0, 0, 0x3FFFFFFF),
        Filter(0x15, 0, 4, 41),
        Filter(0x20, 0, 0, 16),
        Filter(0x15, 1, 0, 2),
        Filter(0x15, 0, 1, 10),
        Filter(0x06, 0, 0, 0x50000 | 13),
    ...
    def __init__(self, root, name, env):
        ...
        try:
            self.call(
                "initialize",
                {
                    "clientInfo": {"name": "s5c-neutral", "version": "1"},
...
def recover_existing(run):
    ...
    root = run / facts[0]["scenario"]
    ident = facts[0]["native_id"]
    env = {
        "PATH": "/usr/bin:/bin",
        "HOME": str(root / "home"),
    ...
    result = {
        "native_id": ident,
        "new_create_calls": 0,
        "original_creation_acknowledged": False,
    }
    try:
        result["resume"] = owner.call(
            "thread/resume", {"threadId": ident, "excludeTurns": True}, allow_error=True
        )
        if "error" not in result["resume"]:
            assert result["resume"]["thread"]["id"] == ident
            result["read"] = owner.call(
                "thread/read",
                {"threadId": ident, "includeTurns": True},
                allow_error=True,
            )
...
def database(native):
    paths = list(native.glob("state_*.sqlite"))
    assert len(paths) == 1, paths
    return paths[0]


def oracle(native, ident, rollout, expected_rows):
    with sqlite3.connect(
        database(native).as_uri() + "?mode=ro", uri=True
    ) as connection:
        connection.row_factory = sqlite3.Row
        rows = [
            dict(row)
            for row in connection.execute(
                "SELECT * FROM threads WHERE id = ?", (ident,)
            )
        ]
    ...
    if expected_rows:
        assert len(records) == 1 and records[0]["type"] == "session_meta", records
        assert records[0]["payload"]["id"] == ident
        assert rows[0]["id"] == ident
    return {"rows": rows, "rollout_records": records}


def main():
    assert platform.system() == "Linux" and platform.machine() == "x86_64"
    run = ROOT / sys.argv[1]
    if len(sys.argv) == 4 and sys.argv[3] == "--recover-existing":
        recover_existing(run)
        return
    assert run.parent == ROOT and not run.exists(), (
        "new owned evidence directory required"
    )
...
""")
            ...
            creator = Owner(root, "creator", env)
            owners.append(creator)
            started = creator.call(
                "thread/start",
                {
                    "cwd": str(root),
                    "ephemeral": False,
            ...
            assert thread["historyMode"] == "paginated" and not thread["ephemeral"], (
                thread
            )
            ident = thread["id"]
            fact = {
            ...
            if mode == "metadata-only-exit":
                read = creator.call(
                    "thread/read", {"threadId": ident, "includeTurns": False}
                )
                assert read["thread"]["id"] == ident
                assert not Path(thread["path"]).exists()
                fact["creator_exit"] = creator.close()
                assert not Path(thread["path"]).exists()
                fact["oracle"] = oracle(native, ident, thread["path"], expected_rows=0)
                continue
            if mode in ["writer-error", "metadata-error"]:
                if mode == "writer-error":
                    Path(thread["path"]).mkdir(parents=True)
                else:
                    db = database(native)
                    with sqlite3.connect(db) as connection:
                        connection.execute(
                            "CREATE TRIGGER injected_metadata_failure BEFORE INSERT ON threads BEGIN SELECT RAISE(FAIL, 'injected metadata failure'); END"
                        )
                error = creator.call(
                    "thread/read",
                    {"threadId": ident, "includeTurns": True},
                ...
                fact["barrier_response"] = error
                assert "error" in error, error
                expected = (
                    "Is a directory"
                    if mode == "writer-error"
                    else "injected metadata failure"
                )
                assert expected in error["error"]["message"], error
                fact["creation_state"] = "failure_propagated_no_ACK"
                if mode == "metadata-error":
                    with sqlite3.connect(database(native)) as connection:
                        connection.execute("DROP TRIGGER injected_metadata_failure")
                    retry = creator.call(
                        "thread/read", {"threadId": ident, "includeTurns": True}
                    )
                    assert (
                        retry["thread"]["id"] == ident
                        and retry["thread"]["turns"] == []
                    )
                    fact["explicit_same_ID_retry_after_fault_removed"] = retry
                    fact["retry_oracle"] = oracle(
                        native, ident, thread["path"], expected_rows=1
                    )
                fact["creator_exit"] = creator.close(abrupt=True)
                fact["oracle"] = oracle(
                    native,
                    ident,
                    thread["path"],
                    expected_rows=1 if mode == "metadata-error" else 0,
                )
                continue
            if mode != "death-before-barrier":
                barrier = creator.call(
                    "thread/read",
                    {"threadId": ident, "includeTurns": True},
                    allow_error=True,
                )
            ...
            if mode != "death-before-barrier":
                ...
                assert (
                    read["thread"]["turns"] == [] and read["thread"]["id"] == ident
                ), read
                fact["fresh_neutral_read"] = read
                fact["fresh_oracle"] = oracle(
                    native, ident, thread["path"], expected_rows=1
                )
            else:
                fact["creation_state"] = (
                    "unacknowledged_exact_ID_recovery_only_no_recreate"
                )
            ...
            json.dumps(
                {
                    ...
                    "network": "seccomp denies IPv4/IPv6 socket creation",
                    "model_turns": 0,
                    "Cutex_invocations": 0,
                    "scope": "process death/restart, NOT fsync/power loss",
                },
                indent=2,
            )
        )
